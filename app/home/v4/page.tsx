        ...
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            ...
            if (!isNaN(idx)) {
              ...
              // Trigger photo pause if auto-scrolling and this is a new photo
              if (autoScrollingRef.current && !pausedByUserRef.current && idx !== lastVisibleIdxRef.current) {
                lastVisibleIdxRef.current = idx;
                isPausedAtPhotoRef.current = true;
                if (photoPauseTimeoutRef.current) clearTimeout(photoPauseTimeoutRef.current);
                photoPauseTimeoutRef.current = setTimeout(() => {
                  isPausedAtPhotoRef.current = false;
                }, 2000);
              }
            }
