...
export default function ChiangMaiCTA(){
  ...
  function handleInstagram(event: React.MouseEvent<HTMLButtonElement>){
    ...
    if (typeof window !== 'undefined' && (window as any).fbq) {
      (window as any).fbq('track', 'Lead', {
        content_name: 'Chiang Mai Free Photo Shoot',
        content_category: 'Instagram Lead'
      })
    }
  ...
  return (
    <div className="mt-6 grid grid-cols-2 gap-3">
      <button
        ...
        className="btn flex items-center justify-center gap-2 px-4 py-2 text-sm text-white sm:px-6"
        style={{ backgroundColor: '#000000' }}
        data-cta="chiang-mai-instagram"
      ...
      >
        ...
        <span>{isSubmitting ? 'Opening…' : 'Instagram'}</span>
      </button>
      <button
      ...
      >
        ...
        <span>{isSubmitting ? 'Opening…' : 'WhatsApp'}</span>
      </button>
    </div>
  )
}
