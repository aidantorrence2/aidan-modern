...
export default function SignUpFormCollabThemes({ selection, onBack, onRestart }: { selection: ThemeSelection | null; onBack?: () => void; onRestart?: () => void }) {
  ...
  useEffect(() => { initPageAnalytics('/sign-up-collab-themes', { version: 'v3', inline: !!onBack, lang: detectLang() }) }, [onBack])
    ...
    try {
      const response = await fetch('/api/sign-up', {
        ...
        body: JSON.stringify({
          ...
          moodboard: ['Collab sign-up', `Location: ${SHOOT.city}`, `Language: ${copy.languageName}`, ...(notes.trim() ? [`Notes: ${notes.trim()}`] : [])],
          photos,
        }),
      })
