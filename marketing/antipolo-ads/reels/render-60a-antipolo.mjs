...
var OUT_DIR = path.join(__dirname, 'output-60a-antipolo');
...
<body>
  <div id="root" style="width:${W}px;height:${H}px;position:relative;overflow:hidden;background:#000;">
    ...
    <div id="vhs-location" style="position:absolute;top:${SAFE_TOP + 60}px;right:50px;z-index:500;display:none;">
      <span class="vhs-ui-text" style="font-size:22px;letter-spacing:4px;opacity:0.7;">ANTIPOLO</span>
    </div>
    ...
    <div id="cta-screen" style="position:absolute;inset:0;z-index:350;display:none;flex-direction:column;align-items:center;justify-content:center;gap:30px;padding:80px;">
      ...
      <div id="cta-location" style="font-size:28px;color:rgba(255,255,255,0.5);font-family:'Courier New',monospace;letter-spacing:6px;text-shadow:1px 0 #ff0000,-1px 0 #0000ff;opacity:0;">ANTIPOLO 2026</div>
    </div>
...
async function main() {
  console.log('=== VHS Tape Reel v60a (Antipolo) ===');
  resetOutputDir();
  ...
  var outputMp4 = path.join(OUT_DIR, '60a-vhs-tape-antipolo.mp4');
  execSync(
  ...
  var reelsDest = path.join(reelsDir, '60a-vhs-tape-antipolo.mp4');
