...
// 5-image carousel set (hook · proof · how · what · cta) — one template, many cities.
// Persistent "<CITY> free photo shoot" wordmark top-right of every slide.
// Add a city below; renders to output-<slug>-5slide/<slug>/.
const CITIES = ['Delhi', 'Varanasi']
...
// embed Poppins so the wordmark's tracked label renders in headless chromium
const fontB64 = f => fs.readFileSync(path.join(FDIR, f)).toString('base64')
const FONTCSS = `@font-face{font-family:'Poppins';font-weight:600;font-style:normal;src:url(data:font/woff2;base64,${fontB64('poppins-700.woff2')}) format('woff2');}`

// persistent corner wordmark (same mark as the story carousels)
...
const badge = name => `<div style="position:absolute;top:52px;right:54px;z-index:60;text-align:right;text-shadow:0 2px 12px rgba(0,0,0,0.95),0 1px 3px rgba(0,0,0,0.9);">
  <div style="font-family:${SERIF};font-size:50px;font-weight:700;letter-spacing:0.15em;color:#fff;line-height:1;">${name.toUpperCase()}</div>
  <div style="display:flex;align-items:center;justify-content:flex-end;gap:15px;margin-top:13px;">
    ...
    <span style="font-family:'Poppins',${SANS};font-size:22px;font-weight:600;letter-spacing:0.28em;color:#fff;">FREE PHOTO SHOOT</span>
  </div>
</div>`
...
function proofSlide(prefix, headline, images) {
  ...
  const imgHtml = images.map((src, i) => {
    const sl = slots[i]
    return `<img src="${src}" style="position:absolute;left:${sl.left}px;top:${sl.top}px;width:${sl.w}px;height:${sl.h}px;object-fit:contain;object-position:center;display:block;transform:rotate(${sl.rot}deg);filter:drop-shadow(0 20px 36px rgba(0,0,0,0.4));"/>`
  }).join('\n        ')
  const inner = `
        <div style="position:absolute;top:300px;left:64px;right:64px;text-align:center;z-index:2;">
...
function buildSlides(city) {
  const prefix = `${slug(city)}-carousel`
  return [
...
async function render() {
  ...
  for (const city of CITIES) {
    ...
    const dir = path.join(__dirname, `output-${s}-5slide`, s)
    ...
    console.log(`${city}: ${slides.length} slides -> ${dir}`)
  }
  await browser.close()
  console.log('Done.')
}
