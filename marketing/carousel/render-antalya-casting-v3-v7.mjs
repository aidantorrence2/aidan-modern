...
// ANTALYA CASTING CALLOUT — five variations (v3..v7) of the 5-slide story set, asked as
...
// bottom ~300px clear for a link sticker.  Run: node render-antalya-casting-v3-v7.mjs [--only=v4]
const CITY = { name: 'Antalya', slug: 'antalya', dates: 'sep 12-13' }
...
const callout = ({ lower, dates }) => ({
  name: '01-callout', html: frame(
    box(260, line(`hi, i'm in ${lower}.`) + line(`looking for models<br/>for tfp collabs ${dates}`, 66, 1, 90) + line('dm me if interested', 66, 1, 90) + line('share 🙂', 66, 1, 90)) +
...
V.v3 = c => [callout(c),
  ...
  { name: '04-free', html: bleed(enc('headliners/000019-6.jpg'), SC_MID, box(1160, line("free. it's tfp.") + line('you keep the full-res scans,<br/>usually within 2 weeks.', 50, 0.95, 24), 'center', 90, 90, SH)) },
  { name: '05-cta', html: bleed(enc('large/manila-gallery-ivy-002.jpg'), SC_LOW, box(1080, line(`dm me "${c.lower}" 🙂`, 72) + line(HANDLE, 54, 0.92, 28) + line(`${c.dates} only`, 46, 0.8, 16), 'center', 80, 80, SH)) },
]
...
V.v4 = c => {
  ...
  return [callout(c),
    ...
    { name: '03-what', html: frame(box(300, line('what you get') + line('a 1-2 hour shoot on 35mm film,<br/>somewhere in kaleiçi or by the water', 50, 0.95, 60) + line('full-res scans in about 2 weeks,<br/>yours to keep', 50, 0.95, 40) + line("free. it's tfp, so i post a few too", 50, 0.95, 40)) +
    ...
    { name: '05-cta', html: frame(box(300, line(`dm me "${c.lower}" 🙂`, 72)) + pr(enc('large/manila-gallery-park-001.jpg'), 260, 520, 560, 840, -2) + box(1440, line(`${HANDLE} · ${c.dates} only`, 46, 0.85))) },
  ]
}

// ── v5 · about me ──────────────────────────────────────────────────────────
V.v5 = c => [callout(c),
  { name: '02-about', html: frame(box(220, line('about me') + line("i'm aidan, a photographer<br/>from the usa.", 50, 0.95, 60) + line("3 years on the road, shooting film.", 50, 0.95, 24) + line(`in ${c.lower} ${c.dates}, then on to kaş.`, 50, 0.95, 24)) + pr(enc('self/aidan-udaipur-mirror-03.jpg'), 340, 960, 400, 560, 2)) },
  ...
  { name: '04-how', html: frame(box(240, line('how it works') + line(`1. dm me "${c.lower}"`, 52, 0.95, 60) + line('2. we plan the look + a spot over dm', 52, 0.95, 34) + line('3. 1-2 hrs shooting on 35mm film', 52, 0.95, 34) + line('4. full-res scans in ~2 weeks', 52, 0.95, 34) + line("free — it's tfp", 52, 0.7, 50), 'left', 100, 100) + pr(enc('large/manila-gallery-tropical-001.jpg'), 660, 1230, 380, 570, 3)) },
  { name: '05-cta', html: bleed(enc('headliners/000020-5.jpg'), SC_LOW, box(1100, line(`dm me "${c.lower}" 🙂`, 72) + line(HANDLE, 54, 0.92, 28) + line(`${c.dates} only`, 46, 0.8, 16), 'left', 100, 100, SH)) },
]
...
V.v6 = c => {
  ...
  return [callout(c), ...vibes.map(vibe),
    { name: '05-cta', html: frame(box(280, line('reply 1, 2 or 3 🙂', 72) + line("and i'll plan a free tfp shoot around it", 46, 0.85, 24)) +
      ...
      box(1250, line(`${HANDLE} · ${c.dates} only`, 46, 0.85))) },
  ]
}
...
V.v7 = c => [callout(c),
  ...
  { name: '03-when', html: frame(box(240, line('when i can shoot') + line('sat 12 sep', 50, 0.95, 60) + line('afternoon + sunset (19:10)', 46, 0.8, 4) + line('sun 13 sep', 50, 0.95, 34) + line('morning, before i leave for kaş', 46, 0.8, 4) + line('1-2 hrs, 35mm film, anywhere central', 50, 0.7, 50)) + pr(enc('headliners/000023.jpg'), 360, 1080, 360, 540, -2)) },
  { name: '04-faq', html: frame(box(200, [['is it free?', "yes — it's tfp, i post a few too"], ['do i need experience?', 'no, i direct every frame'], ['what do i wear?', 'we plan the look together over dm'], ['when do i get the photos?', 'full-res scans in about 2 weeks']].map(q => line(q[0], 44, 0.65, 44) + line(q[1], 52, 1, 6)).join(''), 'left', 100, 100) + pr(enc('headliners/000036-5.jpg'), 740, 1380, 260, 390, 3)) },
