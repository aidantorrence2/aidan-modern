...
function proofSlide(prefix, headline, images, variant = 'dark') {
  ...
  return { name: `${prefix}-02-proof`, html: warmShell(null, inner) }
}
...
function ctaSlide(prefix, city, heroImg, headline, subtext) {
  return {
    name: `${prefix}-05-cta`,
    html: `
      <div style="width:1080px;height:1920px;position:relative;overflow:hidden;background:#000;">
...
function signupSlide(prefix, city, heroImg) {
  return {
    ...
    html: `
      <div style="width:1080px;height:1920px;position:relative;overflow:hidden;background:#000;">
        ...
        <div style="position:absolute;top:0;left:0;right:0;bottom:0;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:80px;">
          ...
          <div style="font-family:${SANS};font-size:30px;color:rgba(255,255,255,0.6);letter-spacing:2px;text-transform:uppercase;margin-bottom:60px;">FREE PHOTO SHOOT — ${city.toUpperCase()}</div>
...
async function render() {
  ...
  for (let i = 0; i < allSlides.length; i++) {
    ...
    // figure out which city folder from the slide name
