...
// CHIANG MAI STORY v2h — v2g with one swap:
//   - 08-cta full-bleed: manila-gallery-ivy-002 → faves/000051-12 (market crates; face clears the CTA text)
//   - (v2g)
//   - 06-casting: DSC_0558 (white bodysuit) → faves/000042-2 (floral dress; clean scan, native aspect)
//   - (v2f)
//   - 04-nervous collage → five faves prints (000016-7, DSC_0869, 000016,
//     000016-3, 000029-3-2); all at native aspect, DSC_0869 crop-trims its scan rebate
const CITY = { name: 'Chiang Mai', slug: 'chiang-mai' }
...
function buildSlides({ name }) {
  ...
  slides.push({
    name: '03-about', html: frame(`
      ...
      <div style="position:absolute;top:400px;left:120px;right:120px;text-align:center;">
        ...
        <p style="font-family:${SE};font-size:36px;color:rgba(255,255,255,0.94);line-height:1.42;margin:24px 0 0;">right now i'm in Thailand, and i'm shooting in ${name}.</p>
  ...
  {
    const collage =
      prx(FV('000016-7.jpg'), 45, 500, 350, 522, -5) +
      prt(FV('DSC_0869.jpg'), 615, 460, 370, 4, 3647, 5780, 45, 30, 3555, 5710) +
      prx(FV('000016.jpg'), 100, 1080, 360, 537, 3) +
      prx(FV('000016-3.jpg'), 605, 1090, 370, 543, -4) +
      prx(FV('000029-3-2.jpg'), 360, 775, 340, 499, 2)
    slides.push({
  ...
  slides.push({
    name: '06-casting', html: frame(`
      ...
      ${prx(FV('000042-2.jpg'), 178, 620, 700, 1039, -1.5)}
    ` + grain(), '#0a0a0a')
  })
  ...
  slides.push(bleed('08-cta', FV('000051-12.jpg'),
...
async function render() {
  ...
  const dir = path.join(__dirname, `output-${CITY.slug}-story-v2h`, CITY.slug)
