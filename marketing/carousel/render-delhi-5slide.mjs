...
// Delhi — original 5-image carousel set (hook · proof · how · what · cta).
// Same template as render-carousel-v2.mjs's per-city loop, city = 'Delhi'.

const __dirname = path.dirname(fileURLToPath(import.meta.url))
const OUT = path.join(__dirname, 'output-delhi-5slide')
...
// -- Content (same as original per-city set) --
...
async function render() {
  ...
  console.log(`Rendering ${allSlides.length} slides for ${city}...`)
  ...
  await browser.close()
  console.log(`\nDone — ${allSlides.length} slides -> ${dir}`)
}
