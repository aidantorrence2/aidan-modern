...
// GOA STORY v2c — v2b with two image swaps:
...
function buildSlides({ name }) {
  ...
  {
    const collage =
      pr(NT('000025-2.jpg'), 45, 500, 350, 520, -5) +
      pr(NT('000010-6.jpg'), 615, 460, 370, 549, 4) +
      pr(NT('000041.jpg'), 100, 1080, 360, 528, 3) +
      pr(FV('000018-8.jpg'), 605, 1090, 370, 552, -4) +
      pr(FV('DSC_0960.jpg'), 360, 775, 340, 520, 2)
    slides.push({
  ...
  slides.push({
    name: '06-casting', html: frame(`
      ...
      ${pr(FV('DSC_0558.jpg'), 178, 620, 700, 921, -1.5)}
    ` + grain(), '#0a0a0a')
  })
...
async function render() {
  ...
  const dir = path.join(__dirname, `output-${CITY.slug}-story-v2c`, CITY.slug)
