...
// GOA STORY v2d — v2b with two image swaps:
...
function buildSlides({ name }) {
  ...
  // 04 — never done this before? five-print staggered collage
  {
    const collage =
      ...
      prx(FV('DSC_0960.jpg'), 360, 775, 340, 520, 2)
    slides.push({
  ...
  slides.push({
    name: '06-casting', html: frame(`
      ...
      ${prx(FV('DSC_0558.jpg'), 178, 620, 700, 921, -1.5)}
    ` + grain(), '#0a0a0a')
  })
...
async function render() {
  ...
  const dir = path.join(__dirname, `output-${CITY.slug}-story-v2d`, CITY.slug)
