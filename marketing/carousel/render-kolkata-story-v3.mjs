...
//   08-proof: REPLACED by v2m-nepal slide 1 (heroHook DSC_0383, 'Kolkata free photo shoot.')
...
function buildSlides({ name }) {
  ...
  // 04 — some of my work (film-print pile; keeper 000016-3 stays put)
  {
  ...
  // 08 — v2m-nepal slide 1 transplant: heroHook DSC_0383 + locked 'Kolkata free photo shoot.'
  slides.push({
    name: '08-proof', html: frame(`
      <img src="${NP('DSC_0383.jpg')}" style="position:absolute;inset:0;width:100%;height:100%;object-fit:cover;object-position:center top;display:block;filter:saturate(1.1) contrast(1.05);"/>
      <div style="position:absolute;inset:0;background:linear-gradient(180deg,rgba(0,0,0,0.08) 0%,transparent 15%,rgba(0,0,0,0.35) 55%,rgba(0,0,0,0.96) 100%);"></div>
      <p style="position:absolute;left:64px;top:64px;font-family:${SA};font-size:24px;font-weight:700;color:rgba(255,255,255,0.7);letter-spacing:0.12em;text-transform:uppercase;margin:0;${SHV}">Kolkata · Shot on 35mm film</p>
      <div style="position:absolute;bottom:540px;left:64px;right:64px;"><p style="font-family:${SE};font-size:108px;font-weight:700;font-style:italic;color:white;line-height:0.92;margin:0;${SHV}">Kolkata<br/>free photo shoot.</p></div>
    ` + grain(0.08), '#000', false)
  })
...
async function render() {
  ...
  const dir = path.join(__dirname, `output-${CITY.slug}-story-v3`, CITY.slug)
