...
//   v3d: all Nepal slots sourced from Exports/nepal headliners (curated set); frames not in
//     it re-picked — pile 0335/0333/0235/0408/0299, triptych 0242/0100/0248, closing 0237
...
function buildSlides({ name }) {
  ...
  // 03 — it's simple (the deal) — v3d: moved after the work pile, five-step body
  slides.push({
    name: '03-deal', html: frame(`
      ...
      <div style="position:absolute;top:410px;left:120px;right:120px;text-align:center;">
        ${["sign up", "we'll choose a location and outfit", "we shoot", "then i'll send you the photos", "and it's totally free"].map((p, i) => `<p style="font-family:${SE};font-size:38px;color:rgba(255,255,255,0.95);line-height:1.3;margin:${i ? '22px' : '0'} 0 0;">${p}</p>`).join('')}
      </div>
      ${photo(H('000019-6.jpg'), 770, 960, 155, 860)}
    ` + grain(), '#0a0a0a')
  })

  // 03 — about me (professional, text above photo)
  slides.push({
    name: '04-about', html: frame(`
  ...
  // 04 — some of my work (film-print pile) — NEPAL HEADLINERS (v3d)
  {
    // v3e: keep the old top-left, top-right, and bottom-left images only; no white mat.
    const f = [NP('DSC_0335.jpg'), NP('DSC_0333.jpg'), NP('DSC_0235.jpg')]
    const pr = (src, l, t, w, h, rot) => `<div style="position:absolute;left:${l}px;top:${t}px;width:${w}px;height:${h}px;transform:rotate(${rot}deg);box-shadow:0 16px 44px rgba(0,0,0,0.55),0 3px 10px rgba(0,0,0,0.3);"><img src="${src}" style="width:${w}px;height:${h}px;object-fit:cover;object-position:center top;display:block;"/></div>`
    const collage =
      pr(f[0], 80, 500, 430, 675, -2) +
      pr(f[1], 570, 500, 430, 675, 2) +
      pr(f[2], 205, 1060, 520, 817, -1)
    slides.push({
  ...
  // 05 — multi-image proof (staggered triptych on black) — NEPAL faves (v3)
  {
    ...
    // v3e: no white mat — the scans carry their own black film borders
    const pr = (src, l, t, w, h, rot) => `<div style="position:absolute;left:${l}px;top:${t}px;width:${w + 22}px;height:${h + 22}px;transform:rotate(${rot}deg);box-shadow:0 14px 40px rgba(0,0,0,0.5),0 3px 9px rgba(0,0,0,0.3);"><img src="${src}" style="width:${w + 22}px;height:${h + 22}px;object-fit:cover;object-position:center top;display:block;"/></div>`
  ...
  slides.push(bleed('10-cta', NP('DSC_0237.jpg'),
...
async function render() {
  ...
  const dir = path.join(__dirname, `output-${CITY.slug}-story-v3e`, CITY.slug)
