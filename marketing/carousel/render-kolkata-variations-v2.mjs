...
// === Kolkata carousel v2 — 5 CREATIVE VARIATIONS ===
// v2 changes: EVERY variant opens with the locked "Kolkata free photo shoot."
// title slide, and every later slide carries a "Kolkata · Free photo shoot"
// tag in the top right. Keeps v1's a (three-steps) and d (chat); replaces the
// other three with new concepts.
//
...
//   f  myth-vs-reality  — what people think a shoot takes vs what this takes
//   g  no-excuses       — every excuse, answered in its own words
//   h  your-job-my-job  — the division of labor: you do three things, I do the rest

const __dirname = path.dirname(fileURLToPath(import.meta.url))
const OUT = path.join(__dirname, 'output-kolkata-variations-v2')
...
// Two stacked lists on dark — myth vs reality, your job vs my job (variants f, h).
// secs = [{label, gold?, items}]
function L_twolists(nm,eyebrow,headline,secs,foot){
  let inner='<div style="position:absolute;left:74px;right:74px;top:300px;"><p style="font-family:'+SA+';font-size:25px;font-weight:700;color:#dcbb7d;letter-spacing:0.14em;text-transform:uppercase;margin:0 0 20px;'+SH+'">'+eyebrow+'</p><p style="font-family:'+SE+';font-size:64px;font-weight:700;font-style:italic;color:#fff;margin:0;line-height:0.95;'+SH+'">'+headline+'</p></div>'
  let top=555
  for(const sec of secs){
    inner+='<p style="position:absolute;left:74px;top:'+top+'px;font-family:'+SA+';font-size:23px;font-weight:700;color:'+(sec.gold?'#dcbb7d':'rgba(255,255,255,0.45)')+';letter-spacing:0.14em;text-transform:uppercase;margin:0;'+SH+'">'+sec.label+'</p>'
    top+=52
    for(const it of sec.items){
      const mark=sec.gold
        ?'<span style="color:#e9c986;font-family:'+SA+';font-weight:700;">&#10003;&nbsp;&nbsp;</span>'
        :'<span style="color:rgba(255,255,255,0.35);font-family:'+SA+';font-weight:700;">&#10005;&nbsp;&nbsp;</span>'
      const col=sec.gold?'#e9c986':'rgba(255,255,255,0.42)'
      inner+='<p style="position:absolute;left:74px;top:'+top+'px;font-family:'+SE+';font-size:46px;font-weight:700;font-style:italic;color:'+col+';margin:0;line-height:1;'+SH+'">'+mark+it+'</p>'
      top+=76
    }
    top+=48
  }
  if(foot)inner+='<div style="position:absolute;left:74px;right:74px;top:'+(top+4)+'px;"><p style="font-family:'+SA+';font-size:26px;color:rgba(255,255,255,0.65);margin:0;line-height:1.4;'+SH+'">'+foot+'</p></div>'
...
// a — THREE STEPS: the manifesto, one giant numbered step per slide. (kept from v1)
function buildA(){
  const p=pick(0)
  return [
    L_hook(p[0],'No prep. No makeup team. Pick an outfit,<br/>pick a location, show up. Sign up below.'),
...
// d — CHAT: the DM conversation everyone actually has with me. (kept from v1, hook fixed)
function buildD(){
  const p=pick(15)
  return [
    L_hook(p[0],'&ldquo;What do I prepare?&rdquo; Nothing.<br/>Outfit, location, show up. Sign up below.'),
...
// f — MYTH VS REALITY: what people think a shoot takes vs what this one takes.
function buildF(){
  const p=pick(3)
  return [
    L_hook(p[0],'Easier than you think — on 35mm film.<br/>Outfit, location, show up. Sign up below.'),
    L_twolists('02','The myth, busted','It&rsquo;s not<br/>that deep.',[
      {label:'What people think it takes', items:['A stylist & makeup chair','Mood boards & fittings','Weeks of back-and-forth']},
      {label:'What it actually takes', gold:true, items:['An outfit you love','A spot in Kolkata','One hour']},
    ],'Daytime, public, and free — we both get the photos.'),
    L_collage('03',p[1],p[2],p[3],{eyebrow:'Recent work', head:'All shot with<br/>zero fuss.', sub:'An outfit, a location, an hour — on 35mm film.', headSz:84, pos:'mid'}),
    L_hero('04',p[4],{eyebrow:'No production', head:'No mood boards.<br/>No makeup chair.', sub:'Just you, your favourite outfit, and good light.', headSz:84, pos:'high'}),
    L_hero('05',p[5],{eyebrow:'Your call', head:'Your outfit.<br/>Your spot.', sub:SPOTS+' — or your own para. Daytime and public.', headSz:92, pos:'low', accent:'gold'}),
    ...
    L_steps('07'),
    L_cta('08',p[8],{eyebrow:'Sign up', head:'See? Not<br/>that deep.', sub:'Quick form below, one quick call, then we shoot. That&rsquo;s the whole production.', headSz:92, pos:'low'}),
  ]
}
...
function buildG(){
  const p=pick(8)
  return [
    L_hook(p[0],'Every excuse, handled — on 35mm film.<br/>Outfit, location, show up. Sign up below.'),
    L_hero('02',p[1],{eyebrow:'Excuse #1', head:'&ldquo;But I&rsquo;m not<br/>photogenic.&rdquo;', sub:'That&rsquo;s my job, not yours. Posing, angles, light — I direct every frame.', headSz:84, pos:'high'}),
    L_hero('03',p[2],{eyebrow:'Excuse #2', head:'&ldquo;I have nothing<br/>to wear.&rdquo;', sub:'You do — the outfit you already love. Your closet is the styling department.', headSz:84, pos:'low', accent:'gold'}),
    L_hero('04',p[3],{eyebrow:'Excuse #3', head:'&ldquo;I&rsquo;ve never<br/>modeled.&rdquo;', sub:'Almost nobody I shoot has. One hour in, you&rsquo;ll forget the camera&rsquo;s there.', headSz:84, pos:'mid'}),
    L_collage('05',p[4],p[5],p[6],{eyebrow:'Recent work', head:'None of them<br/>were models.', sub:'Regular people, one outfit, one hour — on 35mm film.', headSz:84, pos:'mid'}),
    ...
    L_steps('07'),
    L_cta('08',p[9],{eyebrow:'Sign up', head:'Out of<br/>excuses?', sub:'Quick form below — pick an outfit, pick a spot, and show up. I&rsquo;ll handle the rest.', headSz:100, pos:'low'}),
  ]
}

// h — YOUR JOB / MY JOB: the division of labor.
function buildH(){
  const p=pick(12)
  return [
    L_hook(p[0],'Your job: outfit, location, show up.<br/>My job: everything else. Sign up below.'),
    L_twolists('02','Division of labor','Your job.<br/>My job.',[
      {label:'Your job — all of it', gold:true, items:['Pick an outfit','Pick a location','Show up']},
      {label:'My job', items:['Posing & direction','Light, angles, framing','35mm film & editing']},
    ],'You do three things. I do the rest. Free — we both get the photos.'),
    L_collage('03',p[1],p[2],p[3],{eyebrow:'Recent work', head:'My half looks<br/>like this.', sub:'Frames from recent shoots — all on 35mm film.', headSz:84, pos:'mid'}),
    L_hero('04',p[4],{eyebrow:'Your half', head:'You bring<br/>the outfit.', sub:'The one you feel unstoppable in. That&rsquo;s the whole brief.', headSz:88, pos:'high'}),
    L_hero('05',p[5],{eyebrow:'My half', head:'I bring<br/>everything else.', sub:'Camera, film, direction, posing, light. You&rsquo;ll never feel lost.', headSz:88, pos:'low', accent:'gold'}),
    ...
    L_steps('07'),
    L_cta('08',p[7],{eyebrow:'Sign up', head:'Three things.<br/>Then we shoot.', sub:'Outfit, location, show up. Quick form below — I&rsquo;ll handle the rest.', headSz:92, pos:'low'}),
  ]
}
...
const VARIANTS=[
  ...
  {slug:'f-myth-vs-reality', build:buildF},
  {slug:'g-no-excuses', build:buildG},
  {slug:'h-your-job-my-job', build:buildH},
]
