...
// === Kolkata carousel v2b — a-three-steps only ===
// v2b changes vs v2 (a-three-steps): subtitles removed —
//   slide 06: "Scanned 35mm film, color-graded..." sub removed
//   slide 07: all step description lines + "Shooting around Kolkata..." footer removed
//   slide 08: "Quick form, then a quick call..." sub removed
// Everything else identical to v2. Other variants unchanged (see v2).

const __dirname = path.dirname(fileURLToPath(import.meta.url))
const OUT = path.join(__dirname, 'output-kolkata-variations-v2b')
...
// c = {eyebrow,head,sub?,headSz,pos:'high'|'mid'|'low',accent:'gold'} — sub optional in v2b
...
// How it works — v2b: step titles only, no description lines, no footer.
function L_steps(nm){
  ...
  const inner='<div style="position:absolute;left:74px;right:74px;top:300px;"><p style="font-family:'+SA+';font-size:25px;font-weight:700;color:#dcbb7d;letter-spacing:0.14em;text-transform:uppercase;margin:0 0 20px;'+SH+'">How it works</p><p style="font-family:'+SE+';font-size:64px;font-weight:700;font-style:italic;color:#fff;margin:0;line-height:0.95;'+SH+'">Here&rsquo;s how<br/>it works.</p></div>'
    +row('1','Sign up.',560)
    +row('2','Pick your outfit & spot.',730)
    +row('3','Show up. We shoot.',900)
    +row('4','You get the photos.',1070)
...
// a — THREE STEPS: the manifesto, one giant numbered step per slide. (v2b: subs stripped on 06/07/08)
function buildA(){
  const p=pick(0)
  return [
    ...
    L_duo('06',p[7],p[8],{eyebrow:'What you get', head:'Edited photos,<br/>yours to keep.', headSz:84, pos:'mid', accent:'gold'}),
    L_steps('07'),
    L_cta('08',p[9],{eyebrow:'Sign up', head:'Tap the link<br/>below.', headSz:100, pos:'low'}),
  ]
}
