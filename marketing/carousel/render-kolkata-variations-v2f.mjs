...
// === Kolkata carousel v2f — d-chat only ===
...
const OUT = path.join(__dirname, 'output-kolkata-variations-v2f')
...
// Four portrait prints — two top, two bottom, kicker+headline CENTERED in the middle band.
// v2f: symmetric margins (57px cols, top row y90 mirrored by bottom row ending ~y1832),
// copy centered horizontally and vertically between the rows.
function L_collage4(nm,a,b,d,e,c){
  ...
  const copy='<div style="position:absolute;left:74px;right:74px;top:868px;text-align:center;">'+eb
  ...
  return {name:nm,html:dark(
    pr(a,57,90,430,480,-2,12)+pr(b,568,122,430,480,2,12)
    +pr(d,57,1328,430,480,1.5,12)+pr(e,568,1296,430,480,-2,12)
    +copy)}
}
...
function buildD(){
  ...
  return [
    ...
    L_collage4('04',p[1],p[2],p[3],p[4],{eyebrow:'Recent work', head:'the results'}),
