...
// === Kolkata carousel v2g — d-chat only ===
...
const OUT = path.join(__dirname, 'output-kolkata-variations-v2g')
...
// Six portrait prints — three top, three bottom, kicker+headline CENTERED in the middle band.
// v2g: 290x435 prints (35mm 2:3), 35px margins / 34px gutters, gentle stagger per row,
// bottom row mirrors the top margin (ends ~y1830), copy centered between the rows.
function L_collage6(nm,ims,c){
  ...
  return {name:nm,html:dark(
    pr(ims[0],35,90,290,435,-2,12)+pr(ims[1],383,118,290,435,1.5,12)+pr(ims[2],731,94,290,435,2,12)
    +pr(ims[3],35,1371,290,435,1.5,12)+pr(ims[4],383,1343,290,435,-2,12)+pr(ims[5],731,1367,290,435,2,12)
    +copy)}
}
...
function buildD(){
  ...
  return [
    ...
    L_collage6('04',[p[1],p[2],p[3],p[4],p[5],p[6]],{eyebrow:'Recent work', head:'the results'}),
