...
// === Kolkata carousel v2j-nepal — d-chat, NEPAL FAVES image set ===
...
const OUT = path.join(__dirname, 'output-kolkata-variations-v2j-nepal')
...
// Six BIG portrait prints — 2 cols x 3 rows filling the whole canvas (no dead bands),
// kicker+headline dead-center on a soft scrim so the middle row stays visible behind it.
// v2h: 464x520 prints, 40px side margins / 24px gutter, rows y45/673/1301 (84px gaps).
function L_collage6(nm,ims,c){
  const headSz=c.headSz||100
  const eb=c.eyebrow?'<p style="font-family:'+SA+';font-size:25px;font-weight:700;color:#dcbb7d;letter-spacing:0.14em;text-transform:uppercase;margin:0 0 24px;'+SH+'">'+c.eyebrow+'</p>':''
  const scrim='<div style="position:absolute;left:0;right:0;top:610px;height:700px;background:linear-gradient(180deg,transparent,rgba(0,0,0,0.45) 20%,rgba(0,0,0,0.8) 50%,rgba(0,0,0,0.45) 80%,transparent);"></div>'
  const copy='<div style="position:absolute;left:74px;right:74px;top:880px;text-align:center;">'+eb
  ...
  return {name:nm,html:dark(
    pr(ims[0],40,45,464,520,-1.5,12)+pr(ims[1],552,45,464,520,1.5,12)
    +pr(ims[2],40,673,464,520,1,12)+pr(ims[3],552,673,464,520,-1.5,12)
    +pr(ims[4],40,1301,464,520,1.5,12)+pr(ims[5],552,1301,464,520,-1,12)
    +scrim+copy)}
}
...
function buildD(){
  return [
    ...
    L_collage6('04',[NP('DSC_0242.jpg'),NP('DSC_0020-2.jpg'),NP('DSC_0330.jpg'),NP('DSC_0299.jpg'),NP('DSC_0404.jpg'),NP('DSC_0166.jpg')],{eyebrow:'Recent work', head:'the results'}),
