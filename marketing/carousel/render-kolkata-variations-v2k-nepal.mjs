...
// === Kolkata carousel v2k-nepal — d-chat, NEPAL FAVES, photos-first ===
...
const OUT = path.join(__dirname, 'output-kolkata-variations-v2k-nepal')
...
// Six BIG portrait prints — 2 cols x 3 rows, NO SCRIM (photos stay fully visible);
// kicker+headline live in a clean header band ABOVE the grid instead of on the photos.
// v2k: header y64-~200, grid rows y240/788/1336 (488x524 tiles, 40px margins, 24px gutters).
function L_collage6(nm,ims,c){
  ...
  return {name:nm,html:dark(
    copy
    +pr(ims[0],40,240,464,500,-1.5,12)+pr(ims[1],552,240,464,500,1.5,12)
    +pr(ims[2],40,788,464,500,1,12)+pr(ims[3],552,788,464,500,-1.5,12)
    +pr(ims[4],40,1336,464,500,1.5,12)+pr(ims[5],552,1336,464,500,-1,12))}
}

// Full-bleed photo showcase — the image IS the slide. No gradients, no copy; only the
// small top-right campaign badge (added by withBadge) sits over it.
function L_photo(nm,s){return {name:nm,html:'<div style="width:1080px;height:1920px;position:relative;overflow:hidden;background:#000;">'+img(s,0,0,1080,1920,'filter:saturate(1.06) contrast(1.03);')+GR+'</div>'}}
...
function buildD(){
  return [
    ...
    L_photo('05',NP('DSC_0238.jpg')),
    L_photo('06',NP('DSC_0318.jpg')),
    L_photo('07',NP('DSC_0397.jpg')),
