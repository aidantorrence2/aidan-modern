...
// STORY carousel — single source of truth for every city.
// Offer-led spine: hook -> deal -> about -> work -> proofs -> how -> cta.
// Persistent "<CITY> free photo shoot" wordmark top-right of every slide.
// Add a city below and it renders to output-<slug>-story/<slug>/.
const CITIES = [
...
// build the 10 slides for one city (only the location text varies)
function buildSlides({ name }) {
  ...
  // 05 — multi-image proof (staggered triptych on black)
  {
  ...
  // 06–08 — full-bleed proofs (objection-busting caption + nudge)
  ...
  slides.push(bleed('10-cta', H('DSC_0249.jpg'),
    `<div style="position:absolute;top:250px;left:64px;right:64px;text-align:center;">
       ...
       <p style="font-family:${SE};font-size:34px;color:rgba(255,255,255,0.9);margin:34px 0 0;${SH}">A free photo shoot in ${name}. I direct everything.</p>
     </div>`,
