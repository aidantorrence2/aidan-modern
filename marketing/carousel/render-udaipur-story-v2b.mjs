...
// UDAIPUR STORY v2b — clone of render-jaipur-story-v2b.mjs with:
//   - CITY = Udaipur
//   - nepal headliners sourced LOCALLY from public/images/nepal-headliners
//     (copied off PortableSSD so renders no longer need the SSD mounted)
//   - 07-about photographer image = aidan-udaipur-mirror-02.jpg (haveli mirror selfie)
// If the nepal files are missing, the two nepal slides are skipped with a loud
// warning so the rest of the set can still be previewed.
const CITY = { name: 'Udaipur', slug: 'udaipur' }
...
const NEPAL_FILES = ['DSC_0335.jpg', 'DSC_0333.jpg', 'DSC_0235.jpg', 'DSC_0242.jpg', 'DSC_0100.jpg', 'DSC_0248.jpg']
const HAS_NEPAL = NEPAL_FILES.every(f => fs.existsSync(path.join(NEPAL, f)))
...
function buildSlides({ name }) {
  ...
  if (HAS_NEPAL) {
    // 03 — some MORE of my work: nepal film-print pile (no white mat)
    {
      const f = [NP('DSC_0335.jpg'), NP('DSC_0333.jpg'), NP('DSC_0235.jpg')]
      const prn = (src, l, t, w, h, rot) => `<div style="position:absolute;left:${l}px;top:${t}px;width:${w}px;height:${h}px;transform:rotate(${rot}deg);box-shadow:0 16px 44px rgba(0,0,0,0.55),0 3px 10px rgba(0,0,0,0.3);"><img src="${src}" style="width:${w}px;height:${h}px;object-fit:cover;object-position:center top;display:block;"/></div>`
      const collage =
        prn(f[0], 80, 500, 430, 675, -2) +
        prn(f[1], 570, 500, 430, 675, 2) +
        prn(f[2], 205, 1060, 520, 817, -1)
      slides.push({
        name: '03-morework', html: frame(`
          <div style="position:absolute;top:200px;left:60px;right:60px;text-align:center;">${TITLE('some more of my work')}</div>
          ${collage}
        ` + grain(), '#0a0a0a')
      })
    }

    // 04 — never done this before? nepal staggered triptych
    {
      const n = [NP('DSC_0242.jpg'), NP('DSC_0100.jpg'), NP('DSC_0248.jpg')]
      // no white mat — the scans carry their own black film borders
      const prn = (src, l, t, w, h, rot) => `<div style="position:absolute;left:${l}px;top:${t}px;width:${w + 22}px;height:${h + 22}px;transform:rotate(${rot}deg);box-shadow:0 14px 40px rgba(0,0,0,0.5),0 3px 9px rgba(0,0,0,0.3);"><img src="${src}" style="width:${w + 22}px;height:${h + 22}px;object-fit:cover;object-position:center top;display:block;"/></div>`
      const trip = prn(n[1], 305, 520, 470, 620, 1) + prn(n[0], -25, 870, 455, 595, -4) + prn(n[2], 480, 1215, 455, 595, 4)
      slides.push({
  ...
  // 07 — about me (trust/legitimacy; haveli mirror selfie — camera in hand, on location in India)
  slides.push({
    name: '07-about', html: frame(`
      ...
      <img src="${Sf('aidan-udaipur-mirror-02.jpg')}" style="position:absolute;left:310px;top:880px;width:460px;height:580px;object-fit:cover;object-position:center top;display:block;border-radius:8px;"/>
    ` + grain(), '#0a0a0a')
  })

  // 08 — how it works (single reassurance line lives in step 3)
  slides.push({
    name: '08-how', html: frame(`
  ...
  // 09 — CTA (glam full-bleed; honest scarcity, no "act now"; text mid-lower, off the face)
  slides.push(bleed('09-cta', L('manila-gallery-ivy-002.jpg'),
...
async function render() {
  if (!HAS_NEPAL) console.warn('WARNING: nepal-headliners missing from public/images — skipping slides 03-morework & 04-nervous. Copy them from the PortableSSD and re-run for the full 9-card set.')
  ...
  const dir = path.join(__dirname, `output-${CITY.slug}-story-v2b`, CITY.slug)
