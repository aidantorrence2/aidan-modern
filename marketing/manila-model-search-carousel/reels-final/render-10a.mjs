...
const OUT_DIR = path.join(__dirname, "output-10a")
...
function slideOne(images) {
  return `
    <div style="width:${WIDTH}px;height:${HEIGHT}px;position:relative;overflow:hidden;${corkBg()}">
      <!-- MANILA huge -->
      ${manilaTitle(SAFE_LEFT, SAFE_TOP, 82)}

      <!-- Thin accent rule -->
      <div style="position:absolute;left:${SAFE_LEFT}px;top:${SAFE_TOP + 94}px;width:200px;height:3px;background:${MANILA_COLOR};border-radius:2px;z-index:20;"></div>

      <!-- Hero photo pinned center -->
      ${pinnedPhoto(images.hero, 100, SAFE_TOP + 110, 880, 560, -1.2, 'tape')}

      <!-- "models wanted" handwritten note card -->
      ${noteCard('models<br/>wanted', 120, 800, 320, 180, 2.5, '#FFFEF5', HAND, 52)}

      <!-- Small accent photos -->
      ${pinnedPhoto(images.accentA, 520, 820, 240, 300, 1.8, 'pin')}
      ${pinnedPhoto(images.accentB, 780, 790, 220, 280, -2.2, 'pin')}

      <!-- Color swatches cluster -->
      ${colorSwatch(80, 1030, '#D4C4A8', 55, 55)}
      ${colorSwatch(145, 1040, '#C4562A', 55, 55)}
      ${colorSwatch(210, 1030, '#6B4E37', 55, 55)}

      <!-- Small text note -->
      ${noteCard('editorial portraits<br/>no experience needed', 340, 1150, 380, 110, -1, '#FFF8E8', HAND, 26)}

      <!-- Decorative tape strips -->
      ${tapeStrip(750, 1130, 15, 90, 24)}
      ${tapeStrip(60, 1210, -8, 70, 22)}
    </div>
  `
}
...
function slideTwoAnimated(images) {
  ...
  const items = [
    { src: images.gridA, x: 40,  y: SAFE_TOP + 120, w: 310, h: 340, rot: -1.5 },
    { src: images.gridB, x: 380, y: SAFE_TOP + 100, w: 290, h: 320, rot: 2 },
    { src: images.gridC, x: 700, y: SAFE_TOP + 130, w: 320, h: 250, rot: -0.8 },
    { src: images.gridD, x: 60,  y: SAFE_TOP + 480, w: 340, h: 260, rot: 1.3 },
    { src: images.gridE, x: 430, y: SAFE_TOP + 450, w: 280, h: 310, rot: -2 },
    { src: images.gridF, x: 740, y: SAFE_TOP + 400, w: 290, h: 320, rot: 1.5 },
    { src: images.gridG, x: 100, y: SAFE_TOP + 780, w: 300, h: 280, rot: -1 },
    { src: images.gridH, x: 440, y: SAFE_TOP + 800, w: 320, h: 260, rot: 1.8 },
  ]
  ...
  tilesHtml += `
    <div class="pin-tile" style="position:absolute;left:780px;top:${SAFE_TOP + 880}px;width:240px;height:120px;
  ...
  <html>
    ...
    <body>
      <div style="width:${WIDTH}px;height:${HEIGHT}px;position:relative;overflow:hidden;${corkBg()}">
        <!-- MANILA -->
        <div class="header" style="position:absolute;left:${SAFE_LEFT}px;top:${SAFE_TOP}px;z-index:20;">
...
function slideThree(images) {
  ...
  return `
    <div style="width:${WIDTH}px;height:${HEIGHT}px;position:relative;overflow:hidden;${corkBg()}">
      <!-- MANILA -->
      ${manilaTitle(SAFE_LEFT, SAFE_TOP, 80)}
      <div style="position:absolute;left:${SAFE_LEFT}px;top:${SAFE_TOP + 90}px;width:160px;height:3px;background:${MANILA_COLOR};border-radius:2px;z-index:20;"></div>

      <!-- Subheading note -->
      ${noteCard('how it works', SAFE_LEFT, SAFE_TOP + 110, 260, 70, -1, '#FFF8E8', HAND, 30)}

      ${stepsHtml}

      <!-- Small accent photos on right side -->
      ${pinnedPhoto(images.processA, 620, SAFE_TOP + 200, 380, 300, 2, 'pin')}
      ${pinnedPhoto(images.processB, 660, SAFE_TOP + 540, 340, 380, -1.5, 'pin')}

      <!-- Color swatches -->
      ${colorSwatch(80, 1120, MANILA_COLOR, 50, 50)}
      ${colorSwatch(140, 1130, CORK, 50, 50)}
      ${colorSwatch(200, 1120, WARM_BROWN, 50, 50)}

      <!-- Decorative tape -->
      ${tapeStrip(700, 1110, -12, 90, 24)}
    </div>
  `
}
...
function slideFour(images) {
  return `
    <div style="width:${WIDTH}px;height:${HEIGHT}px;position:relative;overflow:hidden;${corkBg()}">
      ...
      ${pinnedPhoto(images.cta, 80, SAFE_TOP + 110, 920, 540, 0.5, 'tape')}

      <!-- Sign up card pinned below hero -->
      ${tapeStrip(340, SAFE_TOP + 670, -1)}
      <div style="position:absolute;left:140px;top:${SAFE_TOP + 680}px;width:800px;height:320px;
      ...
      ${pinnedPhoto(images.ctaAccent, 60, SAFE_TOP + 830, 200, 250, -2.5, 'pin')}
      ${colorSwatch(300, SAFE_TOP + 880, '#D4C4A8', 45, 45)}
      ${colorSwatch(355, SAFE_TOP + 870, MANILA_COLOR, 45, 45)}

      <!-- Decorative tape -->
      ${tapeStrip(800, SAFE_TOP + 850, 10, 80, 22)}
    </div>
  `
}
...
async function render() {
  ...
  const selection = {
    ...
    gridD: 'manila-gallery-garden-001.jpg',
