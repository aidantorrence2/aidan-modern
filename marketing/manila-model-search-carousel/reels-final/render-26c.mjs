...
const OUT_DIR = path.join(__dirname, "output-26c")
...
<body>
  <div id="root" style="width:${WIDTH}px;height:${HEIGHT}px;position:relative;overflow:hidden;background:#0c0c0c;">
    ...
    <!-- Photo grid overlay (shown inline in flow) -->
    <div id="photo-grid" style="display:grid;grid-template-columns:1fr 1fr;gap:10px;padding:0 10px;position:absolute;left:${SAFE_LEFT}px;right:${WIDTH - SAFE_RIGHT}px;z-index:20;opacity:0;">
      ${photoGridHtml}
    </div>
  ...
  <script>
    ...
    // Show persistent header with burn-in
    setTimeout(() => {
    ...
    setTimeout(() => {
      ...
      grid.style.top = Math.min(contentBottom + 10, termRect.top + terminal.scrollHeight + 10) + 'px'
    ...
    // How it works
    addLine('', 11000)
    typeLine(P, './manila --how-it-works', '#fff', 11200, 40, '${MANILA_COLOR}')
    addLine('<span style="color:#22c55e;">Initializing booking sequence...</span>', 12200)
    addLine('<br><span style="color:#facc15;font-weight:700;">[1/3] DM me on Instagram</span>', 12600)
    addLine('<span style="color:#fff;">      Just say hey — I\\'ll reply back</span>', 12800)
    addLine('<span style="color:#555;">      ✓ confirmed</span>', 13100)
    addProgressBar(13200, 400)
    addLine('<br><span style="color:#facc15;font-weight:700;">[2/3] We pick a date</span>', 13700)
    addLine('<span style="color:#fff;">      Location + vibe planned together</span>', 13900)
    addLine('<span style="color:#555;">      ✓ confirmed</span>', 14200)
    addLine('<br><span style="color:#facc15;font-weight:700;">[3/3] Show up. I guide you.</span>', 14600)
    addLine('<span style="color:#fff;">      No posing experience needed</span>', 14800)
    addLine('<span style="color:#555;">      ✓ confirmed</span>', 15100)
    addLine('<br><span style="color:#22c55e;font-weight:700;">✓ All steps complete. Ready to book.</span>', 15500)

    // CTA
    addLine('', 16200)
    typeLine(P, './manila --book-now', '#fff', 16400, 45, '${MANILA_COLOR}')
    addLine('<span style="color:#22c55e;">→ Opening Instagram...</span>', 17300)
    ...
    addLine('<span style="color:#555;">───────────────────────────────────</span>', 18800)
    addLine('<span style="color:#facc15;font-weight:700;">⚡ limited spots this month</span>', 19100)
...
async function render() {
  ...
  writeSources({
    ...
    strategy: 'v26c — single continuous terminal session (one beat, no scene breaks)',
  ...
  const dstVideo = path.join(OUT_DIR, 'manila-terminal-v26c.mp4')

  const { execSync } = await import('child_process')
  try {
    execSync(`ffmpeg -y -i "${srcVideo}" -c:v libx264 -pix_fmt yuv420p -r 30 -an "${dstVideo}"`, { stdio: 'pipe' })
    fs.unlinkSync(srcVideo)
    console.log('Rendered manila-terminal-v26c.mp4')
  } catch (err) {
