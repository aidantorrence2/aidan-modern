...
const OUT_DIR = path.join(__dirname, "output-26d")
...
async function render() {
  ...
  writeSources({
    ...
    strategy: 'v26d — single continuous terminal session (one beat, no scene breaks)',
  ...
  const dstVideo = path.join(OUT_DIR, 'manila-terminal-v26d.mp4')

  const { execSync } = await import('child_process')
  try {
    execSync(`ffmpeg -y -i "${srcVideo}" -c:v libx264 -pix_fmt yuv420p -r 30 -an "${dstVideo}"`, { stdio: 'pipe' })
    fs.unlinkSync(srcVideo)
    console.log('Rendered manila-terminal-v26d.mp4')
  } catch (err) {
