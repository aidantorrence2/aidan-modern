...
const OUT_DIR = path.join(__dirname, "output-28a")
  ...
  const photoSlidesHtml = SLIDESHOW_PHOTOS.map((p, i) => `
    <div id="slide-${i}" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;opacity:0;">
      ...
      <div style="position:absolute;bottom:120px;left:40px;font-family:${MONO};z-index:10;">
        <div style="font-size:30px;letter-spacing:6px;color:${CREAM}, 0.5);text-transform:uppercase;">${p.name}</div>
        <div style="font-size:20px;letter-spacing:8px;color:${CREAM}, 0.25);text-transform:uppercase;margin-top:6px;">${p.city}</div>
      </div>
      <!-- Frame counter -->
  ...
  <div id="root" style="width:${WIDTH}px;height:${HEIGHT}px;position:relative;overflow:hidden;background:#0c0c0c;">
    ...
    <div id="scene1" style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:2;">
      <div id="title-content" style="text-align:center;opacity:0;">
        <div style="font-family:${MONO};font-size:28px;letter-spacing:10px;color:${CREAM},0.35);text-transform:uppercase;margin-bottom:30px;">Manila Presents</div>
        <h1 style="font-family:${MONO};font-size:88px;font-weight:300;letter-spacing:14px;color:${CREAM},0.85);text-transform:uppercase;margin:0;text-shadow:0 0 30px rgba(255,220,150,0.3),0 0 60px rgba(255,200,100,0.1);">Models<br/>Wanted</h1>
        <div style="font-family:${MONO};font-size:24px;letter-spacing:8px;color:${CREAM},0.35);text-transform:uppercase;margin-top:30px;">Editorial Portraits · Free</div>
        <div style="font-family:${MONO};font-size:20px;letter-spacing:6px;color:${CREAM},0.2);text-transform:uppercase;margin-top:16px;">No Experience Needed</div>
      </div>
    </div>
    ...
    <div id="scene2" style="position:absolute;inset:0;z-index:1;opacity:0;">
      ...
      <div style="position:absolute;top:${SAFE_TOP}px;left:0;right:0;text-align:center;z-index:10;pointer-events:none;">
        <div style="font-family:${MONO};font-size:32px;font-weight:300;letter-spacing:10px;color:${CREAM},0.5);text-transform:uppercase;">Aidan Torrence</div>
        <div style="font-family:${MONO};font-size:18px;letter-spacing:6px;color:${CREAM},0.25);text-transform:uppercase;margin-top:6px;">Film Photographer</div>
      </div>
      ${photoSlidesHtml}
    </div>
    ...
    <div id="scene3" style="position:absolute;inset:0;z-index:1;opacity:0;display:flex;flex-direction:column;align-items:center;justify-content:center;">
      <div id="steps-content" style="text-align:center;width:${SAFE_RIGHT - SAFE_LEFT}px;">
        <div style="font-family:${MONO};font-size:28px;letter-spacing:10px;color:${CREAM},0.35);text-transform:uppercase;margin-bottom:20px;opacity:0;" id="steps-pre">How It Works</div>
        <h2 style="font-family:${MONO};font-size:80px;font-weight:300;letter-spacing:10px;color:${CREAM},0.85);text-transform:uppercase;margin:0;text-shadow:0 0 30px rgba(255,220,150,0.2);opacity:0;" id="steps-title">3 Steps</h2>

        <div style="margin-top:60px;text-align:left;padding:0 40px;">
          <div style="margin-bottom:50px;opacity:0;" id="step-1">
            <div style="font-family:${MONO};font-size:22px;letter-spacing:8px;color:${CREAM},0.3);text-transform:uppercase;">Step 01</div>
            <div style="font-family:${MONO};font-size:40px;font-weight:300;letter-spacing:4px;color:${CREAM},0.75);text-transform:uppercase;margin-top:10px;">DM me on Instagram</div>
            <div style="font-family:${MONO};font-size:24px;letter-spacing:4px;color:${CREAM},0.3);margin-top:8px;">Just say hey. I'll reply back.</div>
          </div>

          <div style="margin-bottom:50px;opacity:0;" id="step-2">
            <div style="font-family:${MONO};font-size:22px;letter-spacing:8px;color:${CREAM},0.3);text-transform:uppercase;">Step 02</div>
            <div style="font-family:${MONO};font-size:40px;font-weight:300;letter-spacing:4px;color:${CREAM},0.75);text-transform:uppercase;margin-top:10px;">We pick a date</div>
            <div style="font-family:${MONO};font-size:24px;letter-spacing:4px;color:${CREAM},0.3);margin-top:8px;">Location + vibe planned together.</div>
          </div>

          <div style="opacity:0;" id="step-3">
            <div style="font-family:${MONO};font-size:22px;letter-spacing:8px;color:${CREAM},0.3);text-transform:uppercase;">Step 03</div>
            <div style="font-family:${MONO};font-size:40px;font-weight:300;letter-spacing:4px;color:${CREAM},0.75);text-transform:uppercase;margin-top:10px;">Show up. I guide you.</div>
            <div style="font-family:${MONO};font-size:24px;letter-spacing:4px;color:${CREAM},0.3);margin-top:8px;">No posing experience needed.</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div id="scene4" style="position:absolute;inset:0;z-index:1;opacity:0;display:flex;flex-direction:column;align-items:center;justify-content:center;">
      <div id="cta-content" style="text-align:center;opacity:0;">
        <div style="font-family:${MONO};font-size:24px;letter-spacing:8px;color:${CREAM},0.3);text-transform:uppercase;margin-bottom:20px;">Interested?</div>
        <h2 style="font-family:${MONO};font-size:72px;font-weight:300;letter-spacing:10px;color:${CREAM},0.85);text-transform:uppercase;margin:0;text-shadow:0 0 30px rgba(255,220,150,0.2);">DM Me</h2>
...
async function render() {
  ...
  writeSources({
    ...
    strategy: 'v28a — film projector aesthetic (based on /home/v20) with slideshow + blackout transitions',
  ...
  const dstVideo = path.join(OUT_DIR, 'manila-film-projector-v28a.mp4')

  const { execSync } = await import('child_process')
  try {
    execSync(`ffmpeg -y -i "${srcVideo}" -c:v libx264 -pix_fmt yuv420p -r 30 -an "${dstVideo}"`, { stdio: 'pipe' })
    fs.unlinkSync(srcVideo)
    console.log('Rendered manila-film-projector-v28a.mp4')
  } catch (err) {
