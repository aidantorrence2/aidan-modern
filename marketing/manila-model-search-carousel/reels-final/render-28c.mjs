...
const OUT_DIR = path.join(__dirname, "output-28c")
...
<body>
  <div id="root" style="width:${WIDTH}px;height:${HEIGHT}px;position:relative;overflow:hidden;background:#0c0c0c;">
    ...
    <div id="persistent-title" style="position:absolute;top:${SAFE_TOP + 20}px;left:0;right:0;text-align:center;z-index:45;pointer-events:none;opacity:0;">
      <h1 style="font-family:${MONO};font-size:72px;font-weight:400;letter-spacing:8px;color:${CREAM},0.95);text-transform:uppercase;margin:0;line-height:1.1;text-shadow:0 0 40px rgba(255,220,150,0.4),0 0 80px rgba(255,200,100,0.15);">Manila<br/>Free Photo<br/>Shoot</h1>
    </div>
...
async function render() {
  ...
  writeSources({
    ...
    strategy: 'v28c — film projector aesthetic (based on /home/v20) with more readable text + Manila Free Photo Shoot title',
  ...
  const dstVideo = path.join(OUT_DIR, 'manila-film-projector-v28c.mp4')

  const { execSync } = await import('child_process')
  try {
    execSync(`ffmpeg -y -i "${srcVideo}" -c:v libx264 -pix_fmt yuv420p -r 30 -an "${dstVideo}"`, { stdio: 'pipe' })
    fs.unlinkSync(srcVideo)
    console.log('Rendered manila-film-projector-v28c.mp4')
  } catch (err) {
