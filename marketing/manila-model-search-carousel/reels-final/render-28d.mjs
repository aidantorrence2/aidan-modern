...
const OUT_DIR = path.join(__dirname, "output-28d")
...
<body>
  <div id="root" style="width:${WIDTH}px;height:${HEIGHT}px;position:relative;overflow:hidden;background:#0c0c0c;">
    ...
    <div id="persistent-title" style="position:absolute;top:${SAFE_TOP + 20}px;left:0;right:0;text-align:center;z-index:45;pointer-events:none;opacity:0;">
      <h1 style="font-family:${MONO};font-size:42px;font-weight:400;letter-spacing:6px;color:${CREAM},0.85);text-transform:uppercase;margin:0;white-space:nowrap;text-shadow:0 0 30px rgba(255,220,150,0.3),0 0 60px rgba(255,200,100,0.1);">Manila Free Photo Shoot</h1>
    </div>
    ...
    <div id="scene1" style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:2;">
      <div id="title-content" style="text-align:center;opacity:0;margin-top:200px;">
        <div style="font-family:${MONO};font-size:28px;letter-spacing:6px;color:${CREAM},0.55);text-transform:uppercase;">No Experience Needed</div>
      </div>
    </div>
...
async function render() {
  ...
  writeSources({
    ...
    strategy: 'v28d — film projector aesthetic (based on /home/v20) with more readable text + Manila Free Photo Shoot title',
  ...
  const dstVideo = path.join(OUT_DIR, 'manila-film-projector-v28d.mp4')

  const { execSync } = await import('child_process')
  try {
    execSync(`ffmpeg -y -i "${srcVideo}" -c:v libx264 -pix_fmt yuv420p -r 30 -an "${dstVideo}"`, { stdio: 'pipe' })
    fs.unlinkSync(srcVideo)
    console.log('Rendered manila-film-projector-v28d.mp4')
  } catch (err) {
