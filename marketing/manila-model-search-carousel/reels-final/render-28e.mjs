...
const OUT_DIR = path.join(__dirname, "output-28e")
...
<body>
  <div id="root" style="width:${WIDTH}px;height:${HEIGHT}px;position:relative;overflow:hidden;background:#0c0c0c;">
    ...
    <div id="persistent-title" style="position:absolute;top:${SAFE_TOP + 20}px;left:0;right:0;text-align:center;z-index:45;pointer-events:none;opacity:0;">
      <h1 style="font-family:${MONO};font-size:48px;font-weight:700;letter-spacing:6px;color:${CREAM},1.0);text-transform:uppercase;margin:0;white-space:nowrap;text-shadow:0 0 20px rgba(255,220,150,0.5),0 0 40px rgba(255,200,100,0.25),0 2px 8px rgba(0,0,0,0.7);">Manila Free Photo Shoot</h1>
    </div>
...
async function render() {
  ...
  writeSources({
    ...
    strategy: 'v28e — film projector aesthetic with significantly boosted text readability for phone screens',
    safeBottomPixels: SAFE_BOTTOM,
    photos: SLIDESHOW_PHOTOS.map(p => p.src),
  })

  const browser = await chromium.launch()
  console.log('Recording film projector version (28e — boosted readability)...')
  ...
  const dstVideo = path.join(OUT_DIR, 'manila-film-projector-v28e.mp4')

  const { execSync } = await import('child_process')
  try {
    execSync(`ffmpeg -y -i "${srcVideo}" -c:v libx264 -pix_fmt yuv420p -r 30 -an "${dstVideo}"`, { stdio: 'pipe' })
    fs.unlinkSync(srcVideo)
    console.log('Rendered manila-film-projector-v28e.mp4')
  } catch (err) {
