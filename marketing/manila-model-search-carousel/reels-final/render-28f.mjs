...
const OUT_DIR = path.join(__dirname, "output-28f")
  ...
  const photoSlidesHtml = SLIDESHOW_PHOTOS.map((p, i) => `
    <div id="slide-${i}" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;opacity:0;">
      ...
      <div style="position:absolute;bottom:120px;left:40px;font-family:${MONO};z-index:10;">
        <div style="font-size:40px;font-weight:bold;letter-spacing:6px;color:${CREAM}, 0.95);text-transform:uppercase;text-shadow:0 0 12px rgba(0,0,0,0.9),0 2px 6px rgba(0,0,0,0.8);">${p.name}</div>
        <div style="font-size:30px;letter-spacing:8px;color:${CREAM}, 0.8);text-transform:uppercase;margin-top:6px;text-shadow:0 0 10px rgba(0,0,0,0.9),0 2px 4px rgba(0,0,0,0.7);">${p.city}</div>
      </div>
      <!-- Frame counter -->
      <div style="position:absolute;bottom:120px;right:40px;font-family:${MONO};font-size:30px;font-weight:bold;letter-spacing:4px;color:${CREAM}, 0.75);z-index:10;text-shadow:0 0 10px rgba(0,0,0,0.9),0 2px 4px rgba(0,0,0,0.7);">
...
<body>
  <div id="root" style="width:${WIDTH}px;height:${HEIGHT}px;position:relative;overflow:hidden;background:#0c0c0c;">
    ...
    <!-- Persistent "Manila Free Photo Shoot" title — always on screen, one line -->
    <div id="persistent-title" style="position:absolute;top:${SAFE_TOP + 20}px;left:0;right:0;text-align:center;z-index:45;pointer-events:none;opacity:0;">
      <h1 style="font-family:${MONO};font-size:66px;font-weight:900;letter-spacing:4px;color:${CREAM},1.0);text-transform:uppercase;margin:0;white-space:nowrap;text-shadow:0 0 30px rgba(255,220,150,0.7),0 0 60px rgba(255,200,100,0.4),0 4px 12px rgba(0,0,0,0.9),0 2px 4px rgba(0,0,0,0.8);">Manila Free Photo Shoot</h1>
    </div>
    ...
    <div id="scene1" style="position:absolute;inset:0;display:flex;flex-direction:column;align-items:center;justify-content:center;z-index:2;">
      <div id="title-content" style="text-align:center;opacity:0;margin-top:200px;">
        <div style="font-family:${MONO};font-size:40px;font-weight:bold;letter-spacing:6px;color:${CREAM},0.95);text-transform:uppercase;text-shadow:0 0 15px rgba(255,220,150,0.4),0 2px 6px rgba(0,0,0,0.6);">No Experience Needed</div>
      </div>
    </div>
    ...
    <div id="scene3" style="position:absolute;inset:0;z-index:1;opacity:0;display:flex;flex-direction:column;align-items:center;justify-content:center;">
      <div id="steps-content" style="text-align:center;width:${SAFE_RIGHT - SAFE_LEFT}px;">
        <div style="font-family:${MONO};font-size:36px;font-weight:bold;letter-spacing:10px;color:${CREAM},0.9);text-transform:uppercase;margin-bottom:20px;opacity:0;text-shadow:0 0 12px rgba(255,220,150,0.3),0 2px 4px rgba(0,0,0,0.6);" id="steps-pre">How It Works</div>
        <h2 style="font-family:${MONO};font-size:84px;font-weight:700;letter-spacing:10px;color:${CREAM},1.0);text-transform:uppercase;margin:0;text-shadow:0 0 30px rgba(255,220,150,0.4),0 2px 8px rgba(0,0,0,0.6);opacity:0;" id="steps-title">3 Steps</h2>

        <div style="margin-top:60px;text-align:left;padding:0 40px;">
          <div style="margin-bottom:50px;opacity:0;" id="step-1">
            <div style="font-family:${MONO};font-size:28px;font-weight:bold;letter-spacing:8px;color:${CREAM},0.85);text-transform:uppercase;text-shadow:0 0 10px rgba(255,220,150,0.3);">Step 01</div>
            <div style="font-family:${MONO};font-size:44px;font-weight:700;letter-spacing:4px;color:${CREAM},1.0);text-transform:uppercase;margin-top:10px;text-shadow:0 0 15px rgba(255,220,150,0.3),0 2px 6px rgba(0,0,0,0.5);">DM me on Instagram</div>
            <div style="font-family:${MONO};font-size:30px;letter-spacing:4px;color:${CREAM},0.85);margin-top:8px;text-shadow:0 2px 4px rgba(0,0,0,0.5);">Just say hey. I'll reply back.</div>
          </div>

          <div style="margin-bottom:50px;opacity:0;" id="step-2">
            <div style="font-family:${MONO};font-size:28px;font-weight:bold;letter-spacing:8px;color:${CREAM},0.85);text-transform:uppercase;text-shadow:0 0 10px rgba(255,220,150,0.3);">Step 02</div>
            <div style="font-family:${MONO};font-size:44px;font-weight:700;letter-spacing:4px;color:${CREAM},1.0);text-transform:uppercase;margin-top:10px;text-shadow:0 0 15px rgba(255,220,150,0.3),0 2px 6px rgba(0,0,0,0.5);">We pick a date</div>
            <div style="font-family:${MONO};font-size:30px;letter-spacing:4px;color:${CREAM},0.85);margin-top:8px;text-shadow:0 2px 4px rgba(0,0,0,0.5);">Location + vibe planned together.</div>
          </div>

          <div style="opacity:0;" id="step-3">
            <div style="font-family:${MONO};font-size:28px;font-weight:bold;letter-spacing:8px;color:${CREAM},0.85);text-transform:uppercase;text-shadow:0 0 10px rgba(255,220,150,0.3);">Step 03</div>
            <div style="font-family:${MONO};font-size:44px;font-weight:700;letter-spacing:4px;color:${CREAM},1.0);text-transform:uppercase;margin-top:10px;text-shadow:0 0 15px rgba(255,220,150,0.3),0 2px 6px rgba(0,0,0,0.5);">Show up. I guide you.</div>
            <div style="font-family:${MONO};font-size:30px;letter-spacing:4px;color:${CREAM},0.85);margin-top:8px;text-shadow:0 2px 4px rgba(0,0,0,0.5);">No posing experience needed.</div>
          </div>
        </div>
      </div>
    </div>
    ...
    <div id="scene4" style="position:absolute;inset:0;z-index:1;opacity:0;display:flex;flex-direction:column;align-items:center;justify-content:center;">
      <div id="cta-content" style="text-align:center;opacity:0;">
        <div style="font-family:${MONO};font-size:36px;font-weight:bold;letter-spacing:8px;color:${CREAM},0.9);text-transform:uppercase;margin-bottom:20px;text-shadow:0 0 12px rgba(255,220,150,0.3),0 2px 4px rgba(0,0,0,0.6);">Interested?</div>
        <h2 style="font-family:${MONO};font-size:80px;font-weight:700;letter-spacing:10px;color:${CREAM},1.0);text-transform:uppercase;margin:0;text-shadow:0 0 30px rgba(255,220,150,0.4),0 2px 8px rgba(0,0,0,0.6);">DM Me</h2>
        <div style="margin-top:50px;display:inline-block;border:2px solid ${CREAM},0.5);padding:24px 50px;opacity:0;" id="cta-handle">
          <div style="font-family:${MONO};font-size:52px;font-weight:700;letter-spacing:6px;color:${CREAM},1.0);text-transform:uppercase;text-shadow:0 0 20px rgba(255,220,150,0.4),0 2px 6px rgba(0,0,0,0.5);">@madebyaidan</div>
          <div style="font-family:${MONO};font-size:28px;letter-spacing:6px;color:${CREAM},0.75);text-transform:uppercase;margin-top:10px;text-shadow:0 2px 4px rgba(0,0,0,0.5);">On Instagram</div>
        </div>
        <div style="margin-top:40px;opacity:0;" id="cta-spots">
          <div style="font-family:${MONO};font-size:28px;font-weight:bold;letter-spacing:6px;color:${CREAM},0.8);text-transform:uppercase;text-shadow:0 0 10px rgba(255,220,150,0.3);animation:blinkCursor 2s ease-in-out infinite;">Limited Spots This Month</div>
        </div>
        <div style="font-family:${MONO};font-size:28px;letter-spacing:4px;color:${CREAM},0.7);text-transform:uppercase;margin-top:60px;opacity:0;text-shadow:0 2px 4px rgba(0,0,0,0.5);" id="cta-free">100% Free · No Strings Attached</div>
      </div>
    </div>
  ...
  <script>
    ...
    burnIn('persistent-title', 200)

    // ═══ SCENE 1: SUBTITLE (0–3s) ═══
    burnIn('title-content', 800)
...
async function render() {
  ...
  writeSources({
    ...
    strategy: 'v28f — film projector with baseline readability header (66px/900 weight)',
    safeBottomPixels: SAFE_BOTTOM,
    photos: SLIDESHOW_PHOTOS.map(p => p.src),
  })

  const browser = await chromium.launch()
  console.log('Recording film projector version (28f — baseline readability header)...')
  ...
  const dstVideo = path.join(OUT_DIR, 'manila-film-projector-v28f.mp4')

  const { execSync } = await import('child_process')
  try {
    execSync(`ffmpeg -y -i "${srcVideo}" -c:v libx264 -pix_fmt yuv420p -r 30 -an "${dstVideo}"`, { stdio: 'pipe' })
    fs.unlinkSync(srcVideo)
    console.log('Rendered manila-film-projector-v28f.mp4')
  } catch (err) {
