...
const OUT_DIR = path.join(__dirname, "output-28g")
...
async function render() {
  ...
  writeSources({
    ...
    strategy: 'v28g — film projector with chunky white+red title on dark backing',
    safeBottomPixels: SAFE_BOTTOM,
    photos: SLIDESHOW_PHOTOS.map(p => p.src),
  })

  const browser = await chromium.launch()
  console.log('Recording film projector version (28g — chunky readable title)...')
  ...
  const dstVideo = path.join(OUT_DIR, 'manila-film-projector-v28g.mp4')

  const { execSync } = await import('child_process')
  try {
    execSync(`ffmpeg -y -i "${srcVideo}" -c:v libx264 -pix_fmt yuv420p -r 30 -an "${dstVideo}"`, { stdio: 'pipe' })
    fs.unlinkSync(srcVideo)
    console.log('Rendered manila-film-projector-v28g.mp4')
  } catch (err) {
