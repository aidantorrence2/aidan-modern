...
const OUT_DIR = path.join(__dirname, "output-29a")
...
function buildHTML(imageDataMap) {
  // Timeline:
  // 0–4s     Scene 1: Title "develops" like a print in chemicals
  // 4–14s    Scene 2: Photos develop one by one (dark red → full color)
...
<body>
  <div id="root" style="width:${WIDTH}px;height:${HEIGHT}px;position:relative;overflow:hidden;background:#1a0000;">
    ...
    <!-- ═══ SCENE 1: TITLE ═══ -->
    <div id="scene1" style="position:absolute;inset:0;z-index:10;display:flex;flex-direction:column;align-items:center;justify-content:center;">
      <div id="title-text" style="text-align:center;opacity:0;">
        <div style="font-family:system-ui,sans-serif;font-size:26px;letter-spacing:6px;text-transform:uppercase;font-weight:300;color:#664444;margin-bottom:24px;">Manila Presents</div>
        <h1 style="font-family:system-ui,sans-serif;font-size:96px;font-weight:300;letter-spacing:8px;text-transform:uppercase;color:#cc9999;margin:0;animation:heroGlow 3s ease-in-out infinite;">Models<br/>Wanted</h1>
        <div style="margin-top:32px;font-family:system-ui,sans-serif;font-size:28px;letter-spacing:4px;text-transform:uppercase;font-weight:300;color:#664444;">Editorial Portraits · Free</div>
        <div style="margin-top:16px;font-family:ui-monospace,monospace;font-size:20px;letter-spacing:4px;color:#442222;text-transform:uppercase;">No Experience Needed</div>
      </div>
    </div>

    <!-- ═══ SCENE 2: PHOTO SLIDESHOW ═══ -->
    <div id="scene2" style="position:absolute;inset:0;z-index:5;opacity:0;">
      <!-- Header -->
      <div style="position:absolute;top:${SAFE_TOP}px;left:0;right:0;text-align:center;z-index:10;">
        <div style="font-family:system-ui,sans-serif;font-size:28px;font-weight:300;letter-spacing:6px;color:#cc9999;text-transform:uppercase;">Aidan Torrence</div>
        <div style="font-family:system-ui,sans-serif;font-size:16px;letter-spacing:4px;color:#664444;text-transform:uppercase;margin-top:4px;">Film Photographer</div>
      </div>
      ${photoHtml}
    </div>
  ...
  <script>
    ...
    // ═══ SCENE 1: TITLE (0–4s) ═══
    // Title "develops" like text appearing in chemical bath
    setTimeout(() => {
      const el = document.getElementById('title-text')
      if (el) el.style.animation = 'developReveal 2.5s ease-out forwards'
    }, 300)

    // ═══ SCENE 2: PHOTOS (4–14s) ═══
    setTimeout(() => {
      hide('scene1')
      show('scene2')
    }, 4000)

    // Each photo gets ~1.2s (develop + hold + fade out)
    ${PHOTOS.map((_, i) => `developPhoto(${i}, ${4200 + i * 1200})`).join('\n    ')}
...
async function render() {
  ...
  writeSources({
    ...
    strategy: 'v29a — darkroom reveal aesthetic (based on /home/v12) with chemical develop transitions',
  ...
  const dstVideo = path.join(OUT_DIR, 'manila-darkroom-v29a.mp4')

  const { execSync } = await import('child_process')
  try {
    execSync(`ffmpeg -y -i "${srcVideo}" -c:v libx264 -pix_fmt yuv420p -r 30 -an "${dstVideo}"`, { stdio: 'pipe' })
    fs.unlinkSync(srcVideo)
    console.log('Rendered manila-darkroom-v29a.mp4')
  } catch (err) {
