...
const OUT_DIR = path.join(__dirname, "output-29b")
...
<body>
  <div id="root" style="width:${WIDTH}px;height:${HEIGHT}px;position:relative;overflow:hidden;background:#1a0000;">
    ...
    <!-- Persistent "Manila Free Photo Shoot" title — always on screen -->
    <div id="persistent-title" style="position:absolute;top:${SAFE_TOP + 20}px;left:0;right:0;text-align:center;z-index:45;pointer-events:none;opacity:0;">
      <h1 style="font-family:system-ui,sans-serif;font-size:72px;font-weight:300;letter-spacing:6px;text-transform:uppercase;color:#cc9999;margin:0;line-height:1.1;animation:heroGlow 3s ease-in-out infinite;">Manila<br/>Free Photo<br/>Shoot</h1>
    </div>
    ...
    <div id="scene1" style="position:absolute;inset:0;z-index:10;display:flex;flex-direction:column;align-items:center;justify-content:center;">
      <div id="title-text" style="text-align:center;opacity:0;margin-top:200px;">
        <div style="font-family:system-ui,sans-serif;font-size:28px;letter-spacing:4px;text-transform:uppercase;font-weight:300;color:#664444;">Editorial Portraits · Free</div>
...
async function render() {
  ...
  writeSources({
    ...
    strategy: 'v29b — darkroom reveal aesthetic (based on /home/v12) with chemical develop transitions',
  ...
  const dstVideo = path.join(OUT_DIR, 'manila-darkroom-v29b.mp4')

  const { execSync } = await import('child_process')
  try {
    execSync(`ffmpeg -y -i "${srcVideo}" -c:v libx264 -pix_fmt yuv420p -r 30 -an "${dstVideo}"`, { stdio: 'pipe' })
    fs.unlinkSync(srcVideo)
    console.log('Rendered manila-darkroom-v29b.mp4')
  } catch (err) {
