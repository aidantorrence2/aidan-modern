...
const OUT_DIR = path.join(__dirname, "output-29d")
...
async function render() {
  ...
  writeSources({
    ...
    strategy: 'v29d — darkroom reveal aesthetic (based on /home/v12) with chemical develop transitions',
  ...
  const dstVideo = path.join(OUT_DIR, 'manila-darkroom-v29d.mp4')

  const { execSync } = await import('child_process')
  try {
    execSync(`ffmpeg -y -i "${srcVideo}" -c:v libx264 -pix_fmt yuv420p -r 30 -an "${dstVideo}"`, { stdio: 'pipe' })
    fs.unlinkSync(srcVideo)
    console.log('Rendered manila-darkroom-v29d.mp4')
  } catch (err) {
