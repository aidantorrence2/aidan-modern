...
const OUT_DIR = path.join(__dirname, "output-29e")
    ...
    <div id="photo-wrap-${i}" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;opacity:0;">
      ...
      <div style="position:relative;border:1px solid rgba(140,30,30,0.3);padding:14px;background:rgba(40,5,5,0.3);max-width:90%;max-height:80%;">
        <div style="position:relative;overflow:hidden;line-height:0;">
          <img id="photo-img-${i}" src="${imageDataMap[p.src]}" style="width:100%;height:auto;display:block;max-height:${HEIGHT - SAFE_BOTTOM - SAFE_TOP - 300}px;object-fit:contain;filter:brightness(0.3) saturate(0) sepia(1) hue-rotate(320deg);transition:filter 0.4s cubic-bezier(0.25,0.46,0.45,0.94);"/>
          <!-- Red safelight reflection -->
          <div id="photo-red-${i}" style="position:absolute;inset:0;background:radial-gradient(ellipse at 30% 20%, rgba(180,30,30,0.2) 0%, transparent 50%);pointer-events:none;transition:opacity 0.4s ease;"></div>
          <!-- Chemical wash overlay -->
          <div id="photo-wash-${i}" style="position:absolute;inset:0;overflow:hidden;pointer-events:none;transition:opacity 0.5s ease;">
        ...
        <div style="display:flex;justify-content:space-between;align-items:baseline;margin-top:16px;padding:0 4px;">
          <span style="font-family:Georgia,serif;font-size:32px;color:#fff;letter-spacing:3px;text-transform:uppercase;font-weight:700;font-style:italic;">${p.name}</span>
          <span style="font-family:system-ui,sans-serif;font-size:22px;color:rgba(255,255,255,0.6);letter-spacing:2px;">${p.city}</span>
        </div>
        <!-- Film frame number -->
        <div style="position:absolute;top:-20px;right:14px;font-family:ui-monospace,monospace;font-size:20px;color:rgba(255,255,255,0.3);letter-spacing:2px;">${String(i + 1).padStart(2, '0')}A</div>
      </div>
    </div>
  `).join('')
...
<body>
  <div id="root" style="width:${WIDTH}px;height:${HEIGHT}px;position:relative;overflow:hidden;background:#1a0000;">
    ...
    <!-- Persistent "Manila Free Photo Shoot" title — big bold white italic like reference -->
    <div id="persistent-title" style="position:absolute;bottom:${SAFE_BOTTOM + 60}px;left:${SAFE_LEFT}px;right:${WIDTH - SAFE_RIGHT}px;z-index:45;pointer-events:none;opacity:0;">
      <h1 style="font-family:Georgia,'Times New Roman',serif;font-size:110px;font-weight:900;font-style:italic;color:#fff;margin:0;line-height:1.0;text-shadow:0 2px 20px rgba(0,0,0,0.8);">Manila<br>Free photo<br>shoot</h1>
    </div>

    <!-- Counter -->
    <div id="counter" style="position:absolute;right:40px;bottom:${SAFE_BOTTOM + 30}px;z-index:55;font-family:ui-monospace,monospace;font-size:24px;color:rgba(255,255,255,0.4);letter-spacing:2px;"></div>
    ...
    <div id="scene3" style="position:absolute;inset:0;z-index:5;opacity:0;display:flex;flex-direction:column;justify-content:center;padding:0 ${SAFE_LEFT + 20}px;">
      <div style="opacity:0;" id="steps-head">
        <div style="font-family:system-ui,sans-serif;font-size:28px;letter-spacing:6px;color:rgba(255,255,255,0.5);text-transform:uppercase;font-weight:400;margin-bottom:16px;">How It Works</div>
        <h2 style="font-family:Georgia,serif;font-size:80px;font-weight:900;font-style:italic;color:#fff;margin:0;text-shadow:0 2px 20px rgba(0,0,0,0.5);">3 Steps</h2>
      </div>

      <div style="margin-top:60px;">
        <div style="margin-bottom:50px;opacity:0;border-left:3px solid rgba(255,255,255,0.3);padding-left:28px;" id="s3-step1">
          <div style="font-family:ui-monospace,monospace;font-size:22px;letter-spacing:4px;color:rgba(255,255,255,0.4);text-transform:uppercase;">01</div>
          <div style="font-family:Georgia,serif;font-size:44px;font-weight:700;font-style:italic;color:#fff;margin-top:8px;">DM me on Instagram</div>
          <div style="font-family:system-ui,sans-serif;font-size:28px;color:rgba(255,255,255,0.6);margin-top:8px;">Just say hey. I'll reply back.</div>
        </div>

        <div style="margin-bottom:50px;opacity:0;border-left:3px solid rgba(255,255,255,0.3);padding-left:28px;" id="s3-step2">
          <div style="font-family:ui-monospace,monospace;font-size:22px;letter-spacing:4px;color:rgba(255,255,255,0.4);text-transform:uppercase;">02</div>
          <div style="font-family:Georgia,serif;font-size:44px;font-weight:700;font-style:italic;color:#fff;margin-top:8px;">We pick a date</div>
          <div style="font-family:system-ui,sans-serif;font-size:28px;color:rgba(255,255,255,0.6);margin-top:8px;">Location + vibe planned together.</div>
        </div>

        <div style="opacity:0;border-left:3px solid rgba(255,255,255,0.3);padding-left:28px;" id="s3-step3">
          <div style="font-family:ui-monospace,monospace;font-size:22px;letter-spacing:4px;color:rgba(255,255,255,0.4);text-transform:uppercase;">03</div>
          <div style="font-family:Georgia,serif;font-size:44px;font-weight:700;font-style:italic;color:#fff;margin-top:8px;">Show up. I guide you.</div>
          <div style="font-family:system-ui,sans-serif;font-size:28px;color:rgba(255,255,255,0.6);margin-top:8px;">No posing experience needed.</div>
        </div>
      </div>
    </div>
    ...
    <div id="scene4" style="position:absolute;inset:0;z-index:5;opacity:0;display:flex;flex-direction:column;align-items:center;justify-content:center;">
      <div id="cta-content" style="text-align:center;opacity:0;">
        <div style="font-family:system-ui,sans-serif;font-size:28px;letter-spacing:6px;color:rgba(255,255,255,0.5);text-transform:uppercase;font-weight:400;margin-bottom:20px;">Interested?</div>
        <h2 style="font-family:Georgia,serif;font-size:90px;font-weight:900;font-style:italic;color:#fff;margin:0;text-shadow:0 2px 20px rgba(0,0,0,0.5);">DM Me</h2>

        <div style="margin-top:50px;border:2px solid rgba(255,255,255,0.3);padding:28px 56px;opacity:0;" id="cta-handle">
          <div style="font-family:Georgia,serif;font-size:52px;font-weight:700;font-style:italic;color:#fff;">@madebyaidan</div>
          <div style="font-family:system-ui,sans-serif;font-size:22px;letter-spacing:4px;color:rgba(255,255,255,0.5);text-transform:uppercase;margin-top:10px;">On Instagram</div>
        </div>

        <div style="margin-top:40px;opacity:0;" id="cta-spots">
          <div style="font-family:system-ui,sans-serif;font-size:24px;letter-spacing:4px;color:rgba(255,255,255,0.6);">Limited Spots This Month</div>
        </div>
        <div style="margin-top:24px;font-family:system-ui,sans-serif;font-size:20px;color:rgba(255,255,255,0.35);letter-spacing:3px;opacity:0;" id="cta-free">100% Free · No Strings</div>
      </div>
    </div>
  ...
  <script>
    ...
    function developPhoto(index, startMs) {
      ...
      // Start developing (remove sepia filter) — fast reveal
      setTimeout(() => {
...
async function render() {
  ...
  writeSources({
    ...
    strategy: 'v29e — darkroom reveal aesthetic (based on /home/v12) with chemical develop transitions',
  ...
  const dstVideo = path.join(OUT_DIR, 'manila-darkroom-v29e.mp4')

  const { execSync } = await import('child_process')
  try {
    execSync(`ffmpeg -y -i "${srcVideo}" -c:v libx264 -pix_fmt yuv420p -r 30 -an "${dstVideo}"`, { stdio: 'pipe' })
    fs.unlinkSync(srcVideo)
    console.log('Rendered manila-darkroom-v29e.mp4')
  } catch (err) {
