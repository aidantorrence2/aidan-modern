...
const OUT_DIR = path.join(__dirname, "output-29f")
...
<body>
  <div id="root" style="width:${WIDTH}px;height:${HEIGHT}px;position:relative;overflow:hidden;background:#1a0000;">
    ...
    <div id="persistent-title" style="position:absolute;top:${SAFE_TOP + 20}px;left:0;right:0;text-align:center;z-index:45;pointer-events:none;opacity:0;">
      <h1 style="font-family:system-ui,sans-serif;font-size:52px;font-weight:400;letter-spacing:5px;text-transform:uppercase;color:#f0d0d0;margin:0;white-space:nowrap;animation:heroGlow 3s ease-in-out infinite;">Manila Free Photo Shoot</h1>
    </div>
...
async function render() {
  ...
  writeSources({
    ...
    strategy: 'v29f — darkroom reveal aesthetic (based on /home/v12) with chemical develop transitions',
  ...
  const dstVideo = path.join(OUT_DIR, 'manila-darkroom-v29f.mp4')

  const { execSync } = await import('child_process')
  try {
    execSync(`ffmpeg -y -i "${srcVideo}" -c:v libx264 -pix_fmt yuv420p -r 30 -an "${dstVideo}"`, { stdio: 'pipe' })
    fs.unlinkSync(srcVideo)
    console.log('Rendered manila-darkroom-v29f.mp4')
  } catch (err) {
