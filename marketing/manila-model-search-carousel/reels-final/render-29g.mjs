...
const OUT_DIR = path.join(__dirname, "output-29g")
    ...
    <div id="photo-wrap-${i}" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;opacity:0;">
      ...
      <div style="position:relative;border:1px solid rgba(140,30,30,0.3);padding:14px;background:rgba(40,5,5,0.3);max-width:90%;max-height:80%;">
        ...
        <div style="display:flex;justify-content:space-between;align-items:baseline;margin-top:16px;padding:0 4px;">
          <span style="font-family:system-ui,sans-serif;font-size:28px;color:#ddbbbb;letter-spacing:4px;text-transform:uppercase;font-weight:300;">${p.name}</span>
          <span style="font-family:ui-monospace,monospace;font-size:22px;color:#bb8888;letter-spacing:2px;">${p.city}</span>
        </div>
        <!-- Film frame number -->
  ...
  <div id="root" style="width:${WIDTH}px;height:${HEIGHT}px;position:relative;overflow:hidden;background:#1a0000;">
    ...
    <div id="persistent-title" style="position:absolute;top:${SAFE_TOP + 20}px;left:0;right:0;text-align:center;z-index:45;pointer-events:none;opacity:0;">
      <div style="display:inline-block;background:rgba(0,0,0,0.6);padding:10px 24px;border-radius:4px;border:1px solid rgba(255,255,255,0.1);">
        <span style="font-family:system-ui,-apple-system,sans-serif;font-size:72px;font-weight:900;letter-spacing:3px;text-transform:uppercase;color:#fff;text-shadow:0 2px 4px rgba(0,0,0,0.9),0 0 15px rgba(0,0,0,0.7);">Manila </span><span style="font-family:system-ui,-apple-system,sans-serif;font-size:72px;font-weight:900;letter-spacing:3px;text-transform:uppercase;color:#E8443A;text-shadow:0 2px 4px rgba(0,0,0,0.9),0 0 15px rgba(232,68,58,0.5);">Free</span><br>
        <span style="font-family:system-ui,-apple-system,sans-serif;font-size:72px;font-weight:900;letter-spacing:3px;text-transform:uppercase;color:#fff;text-shadow:0 2px 4px rgba(0,0,0,0.9),0 0 15px rgba(0,0,0,0.7);">Photo Shoot</span>
      </div>
    </div>
    ...
    <div id="scene3" style="position:absolute;inset:0;z-index:5;opacity:0;display:flex;flex-direction:column;justify-content:center;padding:0 ${SAFE_LEFT + 20}px;">
      <div style="opacity:0;" id="steps-head">
        ...
        <h2 style="font-family:system-ui,sans-serif;font-size:80px;font-weight:300;letter-spacing:6px;color:#eecccc;text-transform:uppercase;margin:0;animation:heroGlow 3s ease-in-out infinite;">3 Steps</h2>
      </div>
    ...
    <div id="scene4" style="position:absolute;inset:0;z-index:5;opacity:0;display:flex;flex-direction:column;align-items:center;justify-content:center;">
      <div id="cta-content" style="text-align:center;opacity:0;">
        ...
        <h2 style="font-family:system-ui,sans-serif;font-size:80px;font-weight:300;letter-spacing:8px;color:#eecccc;text-transform:uppercase;margin:0;animation:heroGlow 3s ease-in-out infinite;">DM Me</h2>
...
async function render() {
  ...
  writeSources({
    ...
    strategy: 'v29g — darkroom with chunky white+red readable title',
  ...
  const dstVideo = path.join(OUT_DIR, 'manila-darkroom-v29g.mp4')

  const { execSync } = await import('child_process')
  try {
    execSync(`ffmpeg -y -i "${srcVideo}" -c:v libx264 -pix_fmt yuv420p -r 30 -an "${dstVideo}"`, { stdio: 'pipe' })
    fs.unlinkSync(srcVideo)
    console.log('Rendered manila-darkroom-v29g.mp4')
  } catch (err) {
