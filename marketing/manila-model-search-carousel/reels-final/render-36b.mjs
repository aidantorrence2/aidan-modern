...
var OUT_DIR = path.join(__dirname, 'output-36b');
...
async function main() {
  console.log('=== Manila BTS Lightbox Reel v36b ===');
  resetOutputDir();
  ...
  var finalMp4 = path.join(OUT_DIR, 'manila-bts-lightbox-v36b.mp4');
  execSync(
  ...
  execSync('cp "' + finalMp4 + '" "' + path.join(reelsDir, 'manila-bts-lightbox-v36b.mp4') + '"');

  var sz = statSync(finalMp4);
  console.log('Final: ' + (sz.size / (1024 * 1024)).toFixed(1) + ' MB');
  console.log('Copied to reels/manila-bts-lightbox-v36b.mp4');
  console.log('=== Done ===');
}
