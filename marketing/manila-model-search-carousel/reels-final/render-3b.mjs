...
const OUT_DIR = path.join(__dirname, "output-3b")
...
// Sizing constants — BIGGER for mobile readability (3b: boosted)
const MSG_FONT = 44
const AI_FONT = 44
const THOUGHT_FONT = 32
...
function userBubble(text, id, t) {
  ...
  ">
    <div style="
      ...
      padding:18px 24px;
    ">
      <p style="font-family:${SF};font-size:${MSG_FONT}px;color:#fff;margin:0;line-height:1.4;font-weight:600;">${escaped}</p>
    </div>
  </div>`
}
...
function thinkingIndicator(id, showAt, hideAt) {
  ...
  ">
    <span class="thinking-dots" style="font-family:${SF};font-size:${THOUGHT_FONT}px;color:#ccc;">
      <span style="display:inline-block;width:16px;height:16px;border-radius:50%;background:#ccc;margin-right:5px;animation:dotPulse 1s infinite 0s;"></span>
      <span style="display:inline-block;width:16px;height:16px;border-radius:50%;background:#ccc;margin-right:5px;animation:dotPulse 1s infinite 0.2s;"></span>
      <span style="display:inline-block;width:16px;height:16px;border-radius:50%;background:#ccc;animation:dotPulse 1s infinite 0.4s;"></span>
    </span>
  </div>`
}
...
function thoughtLabel(id, t) {
  ...
  ">
    <span style="font-family:${SF};font-size:${THOUGHT_FONT}px;color:#ccc;font-weight:500;">Thought for a few seconds</span>
  </div>`
}
...
function aiMessage(text, id, t) {
  ...
  return `<div id="${id}" class="msg" style="
    ...
    animation:msgIn 0.35s ease-out ${t}s forwards;
  ">
    <p style="font-family:${SF};font-size:${AI_FONT}px;color:#fff;margin:0;line-height:1.45;font-weight:500;">${escaped}</p>
  </div>`
}
...
function iconRow(id, t) {
  return `<div id="${id}" class="msg" style="
    ...
    animation:msgIn 0.3s ease-out ${t}s forwards;
  ">
    <span style="font-family:${SF};font-size:${ICON_FONT}px;color:#aaa;letter-spacing:4px;">&#128203; &#128266; &#128077; &#128078; &bull;&bull;&bull;</span>
  </div>`
}
...
function searchingStatus(id, t, hideAt) {
  ...
  ">
    <span class="searching-text" style="font-family:${SF};font-size:${THOUGHT_FONT}px;color:#ccc;font-weight:500;">Searching Manila model opportunities &gt;</span>
  </div>`
}
...
function buildHTML(images) {
  ...
  //   user1 bubble (44px font, ~3 lines wrapping): ~200px + 12px = 212px
  //   thought label: ~44px + 6px = 50px
  //   ai1 (44px font, ~5 lines): ~320px + 6px = 326px
  //   icon row: ~40px + 18px = 58px
  //   user2 bubble: ~90px + 12px = 102px
  //   searching: ~44px + 10px = 54px
  //   ai2 (44px font, ~9 lines): ~580px + 6px = 586px
  //   photo1: 750+10 = 760px
  //   photo2: 750+10 = 760px
  //   photo3: 750+10 = 760px
  //   user3 bubble: ~90px + 12px = 102px
  //   ai3 message: ~90px + 6px = 96px
  //   CTA card: ~560px + 20px = 580px
  // Total: ~4446px + 600px padding bottom = ~5046px
  // Max scroll = 5046 - 1430 = ~3616px
  ...
  const scrollKeyframes = `
    ...
    ${p(24)}% { transform: translateY(-4400px); }
...
<html>
  ...
  <body>
    <div style="width:${WIDTH}px;height:${HEIGHT}px;position:relative;overflow:hidden;background:#000;">
      ...
      <!-- Black fill above editorial header (safe zone) -->
      ...
      <div class="phone-frame">
        ...
        <div style="
          ...
          z-index:20;
        ">
          <span style="font-family:${SF};font-size:${HEADER_FONT}px;font-weight:600;color:#fff;letter-spacing:0.01em;">
            Thinking <span style="color:#ccc;font-size:28px;">&rsaquo;</span>
          </span>
        </div>
...
async function render() {
  ...
  fs.writeFileSync(
    ...
    JSON.stringify({ createdAt: new Date().toISOString(), strategy: 'v43d — ChatGPT UI animated conversation (3b: boosted text readability)', images: selection }, null, 2)
  )
