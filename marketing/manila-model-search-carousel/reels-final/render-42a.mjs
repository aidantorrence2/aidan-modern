...
var OUT_DIR = path.join(__dirname, 'output-42a');
...
var CROP_DIR = '/tmp/reel-42a-photos';
...
async function main() {
  console.log('=== "i just wanna be photographerd" reel v42a ===');
  resetOutputDir();
  ...
  var htmlPath = path.join(OUT_DIR, 'reel-42a.html');
  ...
  // Compile to mp4
  var finalMp4 = path.join(OUT_DIR, 'photographer-d-v42a.mp4');
  execSync(
  ...
  execSync('cp "' + finalMp4 + '" "' + path.join(reelsDir, 'photographer-d-v42a.mp4') + '"');

  var sz = statSync(finalMp4);
  console.log('Final: ' + (sz.size / (1024 * 1024)).toFixed(1) + ' MB');
  console.log('Copied to reels/photographer-d-v42a.mp4');
  console.log('=== Done ===');
}
