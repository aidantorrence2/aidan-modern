...
var OUT_DIR = path.join(__dirname, 'output-47a');
...
function buildHTML(imageDataList) {
  ...
  var titleWords = [
    { word: 'BALI',  color: TC.I, delay: 0.3 },
  ...
  /* --- Scene 2: photos fall like Tetris blocks (JS-driven, no CSS animation) --- */
  var photoImgHTML = '';
  ...
  /* Build timeline entries as JS string */
  var tlEntries = [];

  /* Scene 1 shows 0-4s — title blocks + pieces are CSS-driven. We just manage scene visibility. */
...
'  <div style="text-align:center;"><div style="font-size:18px;font-weight:900;color:#00f0f0;letter-spacing:3px;text-shadow:0 0 12px rgba(0,240,240,0.5);">BALI FREE PHOTO SHOOT</div></div>\n' +
...
async function main() {
  console.log('=== Tetris Bali Reel v47a ===');
  resetOutputDir();
  ...
  var finalMp4 = path.join(OUT_DIR, 'manila-tetris-v47a.mp4');
  execSync(
  ...
  execSync('cp "' + finalMp4 + '" "' + path.join(reelsDir, 'manila-tetris-v47a.mp4') + '"');

  var sz = statSync(finalMp4);
  console.log('Final: ' + (sz.size / (1024 * 1024)).toFixed(1) + ' MB');
  console.log('Copied to reels/manila-tetris-v47a.mp4');
  console.log('=== Done ===');
}
