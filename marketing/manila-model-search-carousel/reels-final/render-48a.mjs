...
var OUT_DIR = path.join(__dirname, 'output-48a');
...
'  <div style="text-align:center;"><div style="font-size:18px;font-weight:900;color:#00f0f0;letter-spacing:3px;text-shadow:0 0 12px rgba(0,240,240,0.5);">MANILA FREE PHOTO SHOOT</div></div>\n' +
...
async function main() {
  console.log('=== Tetris Manila/Urban Reel v48a ===');
  resetOutputDir();
  ...
  var finalMp4 = path.join(OUT_DIR, 'manila-tetris-v48a.mp4');
  execSync(
  ...
  execSync('cp "' + finalMp4 + '" "' + path.join(reelsDir, 'manila-tetris-v48a.mp4') + '"');

  var sz = statSync(finalMp4);
  console.log('Final: ' + (sz.size / (1024 * 1024)).toFixed(1) + ' MB');
  console.log('Copied to reels/manila-tetris-v48a.mp4');
  console.log('=== Done ===');
}
