...
var OUT_DIR = path.join(__dirname, 'output-50a');
      ...
      '<div id="photo-wrap-' + i + '" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;opacity:0;">' +
        '<div style="position:relative;border:1px solid rgba(140,30,30,0.3);padding:14px;background:rgba(40,5,5,0.3);max-width:90%;max-height:80%;">' +
          ...
          '<div style="display:flex;justify-content:space-between;align-items:baseline;margin-top:16px;padding:0 4px;">' +
            '<span style="font-family:system-ui,sans-serif;font-size:28px;color:#ddbbbb;letter-spacing:4px;text-transform:uppercase;font-weight:300;">Autumn</span>' +
...
async function main() {
  console.log('=== Darkroom Autumn Woodland reel v50a ===');
  resetOutputDir();
  ...
  var finalMp4 = path.join(OUT_DIR, 'manila-darkroom-v50a.mp4');
  execSync(
  ...
  var reelsDst = path.join(reelsDir, 'manila-darkroom-v50a.mp4');
  execSync('cp "' + finalMp4 + '" "' + reelsDst + '"');

  var sz = statSync(finalMp4);
  console.log('Final: ' + (sz.size / (1024 * 1024)).toFixed(1) + ' MB');
  console.log('Copied to reels/manila-darkroom-v50a.mp4');
  console.log('=== Done ===');
}
