...
var OUT_DIR = path.join(__dirname, 'output-53a');
...
function buildHTML(imageDataMap) {
  ...
  var photoSlidesHtml = PHOTO_NAMES.map(function(name, i) {
    return (
      '<div id="slide-' + i + '" style="position:absolute;inset:0;display:flex;align-items:center;justify-content:center;opacity:0;">' +
        ...
        '<div style="position:absolute;bottom:' + (SAFE_BOTTOM + 30) + 'px;right:40px;font-family:' + MONO + ';font-size:28px;font-weight:bold;letter-spacing:4px;color:' + CREAM + ',0.75);z-index:10;text-shadow:0 0 10px rgba(0,0,0,0.9),0 2px 4px rgba(0,0,0,0.7);">FRAME ' + String(i + 1).padStart(2, '0') + '/' + String(photoCount).padStart(2, '0') + '</div>' +
      '</div>'
    );
  }).join('\n');

  // Build timeline entries as JS string (for __applyUpTo)
  // Scene transitions use blackout: set opacity 1 at transition start, 0 after 0.2s
  // Photo slideshow: 8 photos over 10s (3s-13s), ~1.15s each with 0.1s blackout flash between
  var photoStart = 3.0;
  var photoEnd = 13.0;
  ...
  // Scene 1 start (0s): show scene1, burn in title + subtitle
  timelineEntries.push([0.0, 'showEl("persistent-title"); showEl("scene1"); burnIn("title-content");']);

  // Scene 2 start (3s): blackout flash, hide scene1, show scene2, first photo
  ...
  // Scene 3 start (13s): blackout flash, hide scene2, show scene3
  ...
  // Scene 4 start (18s): blackout flash, hide scene3, show scene4
...
<body>
  <div id="root" style="width:${W}px;height:${H}px;position:relative;overflow:hidden;background:#0c0c0c;">
    ...
    <!-- Persistent title: "Manila Free Photo Shoot" -->
    ...
    <div id="scene3" style="position:absolute;inset:0;z-index:1;opacity:0;display:flex;flex-direction:column;align-items:center;justify-content:center;">
      <div id="steps-content" style="text-align:center;width:${940 - 66}px;">
...
async function main() {
  console.log('=== Film Projector Urban Plaid v53a ===');
  resetOutputDir();
  ...
  for (var frame = 0; frame < TOTAL_FRAMES; frame++) {
    ...
    await page.evaluate(function(time) {
      ...
      var allEls = root.querySelectorAll('[id]');
      allEls.forEach(function(el) {
        var id = el.id;
        if (id === 'blackout') { el.style.opacity = '0'; return; }
        if (id === 'root') return;
        // Persistent overlays and decorations stay visible (no id on them)
      });
      ...
      // Reset title content
      ['title-content','steps-pre','steps-title','steps-content'].forEach(function(id) {
        var el = document.getElementById(id);
  ...
  var finalMp4 = path.join(OUT_DIR, 'manila-film-projector-v53a.mp4');
  execSync(
  ...
  execSync('cp "' + finalMp4 + '" "' + path.join(reelsDir, 'manila-film-projector-v53a.mp4') + '"');

  var sz = statSync(finalMp4);
  console.log('Final: ' + (sz.size / (1024 * 1024)).toFixed(1) + ' MB');
  console.log('Copied to reels/manila-film-projector-v53a.mp4');
  console.log('=== Done ===');
}
