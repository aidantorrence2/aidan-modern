...
var OUT_DIR = path.join(__dirname, 'output-55a');
...
async function main() {
  console.log('=== Film Projector Christmas Holiday v55a ===');
  resetOutputDir();
  ...
  var finalMp4 = path.join(OUT_DIR, 'manila-film-projector-v55a.mp4');
  execSync(
  ...
  execSync('cp "' + finalMp4 + '" "' + path.join(reelsDir, 'manila-film-projector-v55a.mp4') + '"');

  var sz = statSync(finalMp4);
  console.log('Final: ' + (sz.size / (1024 * 1024)).toFixed(1) + ' MB');
  console.log('Copied to reels/manila-film-projector-v55a.mp4');
  console.log('=== Done ===');
}
