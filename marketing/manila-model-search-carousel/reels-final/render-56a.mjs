...
var OUT_DIR = path.join(__dirname, 'output-56a');
...
function buildHTML(imageDataMap) {
  ...
  // Timeline events as data — each has a time (seconds) and type
  // Types: addLine, typingLine, showHeader, showOverlay, showSlide, hideOverlay, showCTA, showCursor
  // For frame capture, we rebuild the terminal from scratch on each frame using __applyUpTo
...
<script>
  ...
  // Timeline: array of { time, fn }
  // fn receives current time t for interpolation
  var timeline = [
    // 0.0s — header fade in
    ...
    // 0.5s — typing: "> find me a free photo shoot in manila"
    // charMs = 35ms per char → full line at 0.5 + 38*0.035 = ~1.83s
    { time: 0.5, fn: function(t) {
      var text = 'find me a free photo shoot in manila';
      var charMs = 0.035; // seconds per char
      var elapsed = t - 0.5;
    ...
    // 3.2s — remove searching, show results
  ...
  window.__applyUpTo = function(t) {
    ...
    // Apply all timeline events up to time t
    ...
    // Remove searching line if past 3.2s
    if (t >= 3.2) {
...
async function main() {
  console.log('=== Claude Code Night Escalator Reel v56a ===');
  resetOutputDir();
  ...
  var outputMp4 = path.join(OUT_DIR, 'manila-claude-code-v56a.mp4');
  execSync(
  ...
  execSync('cp "' + outputMp4 + '" "' + path.join(reelsDir, 'manila-claude-code-v56a.mp4') + '"');

  var sz = statSync(outputMp4);
  console.log('Final: ' + (sz.size / (1024 * 1024)).toFixed(1) + ' MB');
  console.log('Copied to reels/manila-claude-code-v56a.mp4');
  console.log('=== Done ===');
}
