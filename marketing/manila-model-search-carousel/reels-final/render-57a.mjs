...
var OUT_DIR = path.join(__dirname, 'output-57a');
...
async function main() {
  console.log('=== Claude Code European Street Fashion Reel v57a ===');
  resetOutputDir();
  ...
  var outputMp4 = path.join(OUT_DIR, 'manila-claude-code-v57a.mp4');
  execSync(
  ...
  execSync('cp "' + outputMp4 + '" "' + path.join(reelsDir, 'manila-claude-code-v57a.mp4') + '"');

  var sz = statSync(outputMp4);
  console.log('Final: ' + (sz.size / (1024 * 1024)).toFixed(1) + ' MB');
  console.log('Copied to reels/manila-claude-code-v57a.mp4');
  console.log('=== Done ===');
}
