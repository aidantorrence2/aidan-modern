...
<div id="root" style="width:${W}px;height:${H}px;position:relative;overflow:hidden;background:#0d0d1a;">
  ...
  <div id="marquee" style="position:absolute;top:${SAFE_TOP + 10}px;left:50%;transform:translateX(-50%);width:880px;z-index:50;text-align:center;opacity:0;">
    <div style="background:linear-gradient(180deg,#2a1a00,#1a1000);border:4px solid #FFD700;border-radius:20px;padding:20px 40px;box-shadow:0 0 30px rgba(255,215,0,0.4);">
      ...
      <div style="font-size:42px;font-weight:900;color:#FFD700;text-transform:uppercase;letter-spacing:4px;text-shadow:0 0 20px rgba(255,215,0,0.8);line-height:1.15;">MANILA FREE<br>PHOTO SHOOT</div>
  ...
  ${(function() {
    var coins = '';
    for (var i = 0; i < 30; i++) {
      coins += '<div id="coin-' + i + '" style="position:absolute;width:36px;height:36px;border-radius:50%;background:radial-gradient(circle at 40% 40%,#ffd700,#daa520,#b8860b);border:2px solid #8B6914;z-index:180;opacity:0;font-size:16px;font-weight:900;color:#8B6914;display:flex;align-items:center;justify-content:center;box-shadow:0 2px 8px rgba(0,0,0,0.4);">$</div>';
    }
    return coins;
  })()}
  ...
  <div id="cta-panel" style="position:absolute;inset:0;z-index:250;background:#0d0d1a;opacity:0;pointer-events:none;">
    <div style="position:absolute;top:0;left:0;right:0;bottom:0;display:flex;flex-direction:column;align-items:center;justify-content:center;padding:60px;">
      ...
      <div id="cta-loc" style="margin-top:20px;font-size:22px;color:rgba(255,255,255,0.45);letter-spacing:2px;text-transform:uppercase;text-align:center;opacity:0;">limited slots &middot; manila sessions</div>
    </div>
    <!-- CTA sparkles -->
...
<script>
  ...
  for (var ci = 0; ci < 30; ci++) {
    ...
    var dist = 200 + seededRandom(ci * 13 + 3) * 600;
    coinData.push({
      startX: 540, startY: 800,
      endX: 540 + Math.cos(angle) * dist,
      endY: 800 + Math.sin(angle) * dist - 300,
      delay: seededRandom(ci * 11 + 5) * 0.6,
      rotation: seededRandom(ci * 17 + 7) * 720 - 360,
    });
  }
  ...
  window.__applyUpTo = function(t) {
    ...
    for (var ci2 = 0; ci2 < 30; ci2++) {
      ...
      if (t < coinStart || t >= coinEnd) {
        ...
        coin.style.opacity = String(cp < 0.75 ? 1 : lerp(1, 0, prog(cp, 0.75, 1.0)));
        var ex = lerp(cd.startX, cd.endX, easeOut(cp));
        var ey = lerp(cd.startY, cd.endY, easeOut(cp)) + cp * cp * 500;
        coin.style.left = (ex - 18) + 'px';
        coin.style.top = (ey - 18) + 'px';
        coin.style.transform = 'rotate(' + (cd.rotation * cp) + 'deg)';
      }
    }
...
async function main() {
  console.log('=== Manila Slot Machine Reel v59a ===');
  resetOutputDir();
