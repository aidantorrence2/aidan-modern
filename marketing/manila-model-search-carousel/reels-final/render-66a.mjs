...
var OUT_DIR = path.join(__dirname, 'output-66a');
...
<div id="root" class="page">
  ...
  <div class="strips-container" id="strips-container">
    ...
    <div class="photo-strip" id="strip-1" style="opacity:0;transform:translateY(${H + 200}px);">
      <div class="strip-header">MANILA FREE PHOTO SHOOT</div>
      ...
      <div class="strip-footer">@madebyaidan • manila 2026</div>
    </div>
  ...
  <div class="text-strip" id="text-strip">
    ...
    <div class="text-strip-slot" id="tslot-3">
      ...
      <div class="text-slot-accent" id="tacc-3">look cute</div>
    </div>
  ...
  <!-- Final social strip (18-22s) -->
  <div class="final-strip" id="final-strip">
    <div class="strip-header">FIND ME ON INSTAGRAM</div>
    <div class="final-slot" id="fslot-1">
      <div class="final-slot-text" id="ftext-1">@madebyaidan</div>
      <div class="final-slot-sub" id="fsub-1">instagram</div>
    </div>
    <div class="final-slot" id="fslot-2">
      <div class="final-slot-text" id="ftext-2">DM for booking</div>
      <div class="final-slot-sub" id="fsub-2">it's free!</div>
    </div>
    <div class="final-slot" id="fslot-3">
      <div class="final-slot-text" id="ftext-3">Limited spots</div>
      <div class="final-slot-sub" id="fsub-3">book now ♡</div>
    </div>
    <div class="final-slot" id="fslot-4">
      <div class="final-slot-text" id="ftext-4">MANILA 2026</div>
      <div class="final-slot-sub" id="fsub-4">free photo shoot</div>
    </div>
    <div class="strip-footer">@madebyaidan • manila free shoot</div>
  </div>
...
<script>
  ...
  // Final strip elements
  var ftexts = [
    document.getElementById('ftext-1'),
    document.getElementById('ftext-2'),
    document.getElementById('ftext-3'),
    document.getElementById('ftext-4'),
  ];
  var fsubs = [
    document.getElementById('fsub-1'),
    document.getElementById('fsub-2'),
    document.getElementById('fsub-3'),
    document.getElementById('fsub-4'),
  ];
  ...
  window.__applyUpTo = function(t) {
    ...
    // Final strip hidden
    finalStrip.style.opacity = '0';
    finalStrip.style.transform = 'translateY(' + (H + 200) + 'px)';
    for (var m = 0; m < 4; m++) {
      ftexts[m].style.opacity = '0';
      fsubs[m].style.opacity = '0';
    }
    ...
    // ============================================================
    // 18–22s: Final strip with social info
    // ============================================================
    if (t >= 18.0 && t < 22.0) {
      // Text strip still visible briefly, then fades
      if (t < 18.3) {
        var tsInP2 = progress(t, 18.0, 18.3);
        textStrip.style.transform = 'translateY(' + lerp(0, -(H + 200), easeIn(tsInP2)) + 'px)';
        for (var ti = 0; ti < 4; ti++) {
          ttexts[ti].style.opacity = String(lerp(1, 0, tsInP2));
          taccs[ti].style.opacity = String(lerp(1, 0, tsInP2));
        }
      }

      // Flash at 18.0
      if (t >= 18.0 && t < 18.3) {
        var finFlashP = progress(t, 18.0, 18.3);
        var finFlashVal = finFlashP < 0.25 ? finFlashP / 0.25 : 1 - (finFlashP - 0.25) / 0.75;
        flashOverlay.style.opacity = String(finFlashVal * 0.9);
      }

      // Final strip slides in
      if (t >= 18.2) {
        var finInP = progress(t, 18.2, 18.8);
        finalStrip.style.opacity = '1';
        finalStrip.style.transform = 'translateY(' + lerp(H + 200, 0, easeOut(finInP)) + 'px)';
      } else {
        finalStrip.style.opacity = '0';
        finalStrip.style.transform = 'translateY(' + (H + 200) + 'px)';
      }
      ...
      // Social info reveals
      if (t >= 18.9) {
        ftexts[0].style.opacity = String(easeOut(progress(t, 18.9, 19.3)));
        fsubs[0].style.opacity = String(easeOut(progress(t, 19.1, 19.5)));
      }
      if (t >= 19.5) {
        ftexts[1].style.opacity = String(easeOut(progress(t, 19.5, 19.9)));
        fsubs[1].style.opacity = String(easeOut(progress(t, 19.7, 20.1)));
      }
      if (t >= 20.1) {
        ftexts[2].style.opacity = String(easeOut(progress(t, 20.1, 20.5)));
        fsubs[2].style.opacity = String(easeOut(progress(t, 20.3, 20.7)));
      }
      if (t >= 20.7) {
        ftexts[3].style.opacity = String(easeOut(progress(t, 20.7, 21.1)));
        fsubs[3].style.opacity = String(easeOut(progress(t, 20.9, 21.3)));
      }
    }
    ...
    // ============================================================
    if (t >= 22.0) {
      // Keep final strip visible
      finalStrip.style.opacity = '1';
      finalStrip.style.transform = 'translateY(0)';
      for (var fi = 0; fi < 4; fi++) {
        ftexts[fi].style.opacity = '1';
        fsubs[fi].style.opacity = '1';
      }

      // Flash burst at 22.0
      if (t >= 22.0 && t < 22.5) {
        var lastFlashP = progress(t, 22.0, 22.5);
        var lastFlashVal = lastFlashP < 0.2 ? lastFlashP / 0.2 : 1 - (lastFlashP - 0.2) / 0.8;
        endFlash.style.opacity = String(lastFlashVal);
      }

      // Hold: white screen from 22.5 to 23s
      if (t >= 22.4 && t < 23.0) {
        endFlash.style.opacity = String(lerp(0, 0.95, progress(t, 22.4, 22.6)));
      }

      // Fade out to final card
      if (t >= 23.0) {
        endFlash.style.opacity = String(lerp(0.95, 0, progress(t, 23.0, 23.8)));
      }
    }
...
async function main() {
  console.log('=== Photo Booth Strip v66a ===');
  resetOutputDir();
  ...
  var outputMp4 = path.join(OUT_DIR, 'manila-photobooth-v66a.mp4');
  execSync(
  ...
  var reelsDest = path.join(reelsDir, 'manila-photobooth-v66a.mp4');
