...
var OUT_DIR = path.join(__dirname, 'output-66e');
...
function processPhotos() {
  ...
  var cacheDir = path.join(__dirname, 'output-66a', 'tmp-photos');
  var processed = {};
  for (var name of PHOTO_NAMES) {
    ...
    if (existsSync(cached)) {
      ...
      if (!existsSync(src)) {
        console.error('Photo not found (no cache, no SSD): ' + name);
        process.exit(1);
      }
...
async function main() {
  console.log('=== Photo Booth Strip v66e ===');
  resetOutputDir();
  ...
  var outputMp4 = path.join(OUT_DIR, '66e-photo-booth.mp4');
  execSync(
  ...
  var reelsDest = path.join(reelsDir, '66e-photo-booth.mp4');
