...
var OUT_DIR = path.join(__dirname, 'output-82a');
...
function loadPhotos() {
  ...
  for (var name of PHOTO_NAMES) {
    var src = path.join(FAVES_DIR, name);
    if (!existsSync(src)) {
...
<body>
  <div id="root" style="width:${W}px;height:${H}px;position:relative;overflow:hidden;background:#1a1a2e;">
    ...
    <div id="persistent-header" style="position:absolute;top:${SAFE_TOP}px;left:${SAFE_LEFT}px;width:${SAFE_RIGHT - SAFE_LEFT}px;z-index:50;pointer-events:none;opacity:0;padding:12px 0;border-bottom:1px solid #333;">
      <div style="font-family:${MONO};font-size:26px;font-weight:700;color:${CLAUDE_ORANGE};letter-spacing:2px;text-transform:uppercase;">Bali Model Collab</div>
    </div>
...
<script>
  ...
  var timeline = [
    ...
    { time: 0.5, fn: function(t) {
      var text = 'find me a model collab in bali';
      var charMs = 0.035;
      var elapsed = t - 0.5;
    ...
    { time: 3.2, fn: function(t) {
      content.appendChild(makeLineDiv('<span style="color:#22c55e;">&#10003; Found: Bali Model Collab</span>'));
    }},
    ...
    { time: 4.0, fn: function(t) {
      content.appendChild(makeLineDiv('<span style="color:' + CLAUDE_AMBER + ';font-weight:700;">  cost:       FREE (TFP collab)</span>'));
    }},
    ...
    { time: 13.2, fn: function(t) {
      content.appendChild(makeLineDiv('<span style="color:' + CLAUDE_ORANGE + ';">&#x27F3; How to collab &#8212; 3 steps:</span>'));
    }},
    { time: 13.6, fn: function(t) {
      content.appendChild(makeLineDiv('<br><span style="color:' + CLAUDE_AMBER + ';font-weight:700;">[1/3] Click the link below</span>'));
    }},
    { time: 13.8, fn: function(t) {
      content.appendChild(makeLineDiv('<span style="color:#e5e7eb;">      Sign up &#8212; takes 30 seconds</span>'));
    }},
    ...
    { time: 15.8, fn: function(t) {
      content.appendChild(makeLineDiv('<br><span style="color:#22c55e;font-weight:700;">&#10003; All steps complete. Ready to collab.</span>'));
    }},
    ...
    { time: 16.3, fn: function(t) {
      ...
      box.innerHTML = '<p style="font-size:50px;font-weight:700;color:#fff;margin:0 0 8px;">@madebyaidan</p><p style="font-size:26px;color:rgba(255,255,255,0.5);margin:0 0 22px;">Bali Model Collab</p><div style="display:inline-block;background:' + CLAUDE_ORANGE + ';border-radius:12px;padding:16px 36px;"><span style="font-size:28px;font-weight:700;color:#fff;">Click the link below to sign up &#8594;</span></div>';
...
async function main() {
  console.log('=== Bali Model Collab Terminal Reel v82a ===');
  resetOutputDir();

  console.log('Loading photos...');
  var imageDataMap = loadPhotos();
  ...
  var outputMp4 = path.join(OUT_DIR, 'bali-terminal-v82a.mp4');
  execSync(
  ...
  execSync('cp "' + outputMp4 + '" "' + path.join(reelsDir, '82a-bali-terminal.mp4') + '"');

  var sz = statSync(outputMp4);
  console.log('Final: ' + (sz.size / (1024 * 1024)).toFixed(1) + ' MB');
  console.log('Copied to reels/82a-bali-terminal.mp4');
  console.log('=== Done ===');
}
