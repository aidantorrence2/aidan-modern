...
const OUT_DIR = path.join(__dirname, 'output-v15')
...
function topLabel(dark = false) {
  return `
    <div style="position:absolute;left:58px;top:56px;right:58px;display:flex;align-items:center;justify-content:space-between;">
...
function cleanImage(src, style, fit = 'cover', radius = 34, objectPos = 'center') {
  return `
    <div style="position:absolute;${style};overflow:hidden;border-radius:${radius}px;box-shadow:0 28px 58px rgba(0,0,0,0.18);">
      <img src="${src}" style="width:100%;height:100%;display:block;object-fit:${fit};object-position:${objectPos};"/>
    </div>
  `
}
...
function darkBox({ left, top, width, title, body }) {
  return `
    <div style="position:absolute;left:${left}px;top:${top}px;width:${width}px;padding:30px 34px 28px;border-radius:34px;background:rgba(6,10,15,0.9);border:1px solid rgba(255,255,255,0.12);box-shadow:0 28px 60px rgba(0,0,0,0.24);">
      <h1 style="font-family:${DISPLAY};font-size:84px;font-weight:700;line-height:0.92;color:#fff;margin:0 0 16px;">${title}</h1>
      <p style="font-family:${SANS};font-size:31px;line-height:1.32;color:rgba(243,246,252,0.92);margin:0;">${body}</p>
    </div>
  `
}
  ...
  return `
    <div style="width:${WIDTH}px;height:${HEIGHT}px;position:relative;overflow:hidden;background:#efe6dc;">
      ...
      ${darkBox({
        ...
        top: 148,
        width: 964,
        title: 'Models wanted in Manila.',
        body: 'Editorial portrait shoots. No experience needed. Sign up below.'
      })}
      ${cleanImage(images.hero, 'left:42px;top:568px;width:996px;height:1310px;', 'cover', 42, 'top 15%')}
    </div>
  `
}
...
function slideTwo(images) {
  return `
    <div style="width:${WIDTH}px;height:${HEIGHT}px;position:relative;overflow:hidden;background:#efe6dc;">
      ...
      ${framed(images.keepOne, 'left:58px;top:432px;width:316px;height:430px;transform:rotate(-2deg);')}
      ${framed(images.keepTwo, 'left:386px;top:432px;width:316px;height:430px;transform:rotate(2deg);')}
      ${framed(images.proofA, 'left:706px;top:432px;width:316px;height:430px;transform:rotate(-1deg);')}
      ${framed(images.proofB, 'left:58px;top:896px;width:472px;height:704px;')}
      ${framed(images.proofC, 'left:550px;top:896px;width:472px;height:704px;')}
    </div>
  `
}
...
function slideThree(images) {
  return `
    <div style="width:${WIDTH}px;height:${HEIGHT}px;position:relative;overflow:hidden;background:#f1e6d9;">
      ...
      ${chip('Guided direction so first-timers feel comfortable.', 544, 1052, 420)}
      ${chip('Edited photos ready for portfolio or social.', 544, 1196, 420)}
      ${chip('Help with vibe, styling, and overall look.', 544, 1340, 420)}
...
function slideFive(images) {
  return `
    <div style="width:${WIDTH}px;height:${HEIGHT}px;position:relative;overflow:hidden;background:#0f1721;">
      ...
      ${safeShade(true)}
      ${topLabel(true)}
      <div style="position:absolute;left:58px;top:148px;width:964px;padding:30px 34px 28px;border-radius:34px;background:rgba(6,10,15,0.9);border:1px solid rgba(255,255,255,0.12);box-shadow:0 30px 62px rgba(0,0,0,0.24);">
        <h2 style="font-family:${DISPLAY};font-size:82px;font-weight:700;line-height:0.9;color:#fff;margin:0 0 16px;">Sign up — it takes 60 seconds.</h2>
        <p style="font-family:${SANS};font-size:30px;line-height:1.34;color:rgba(243,246,252,0.92);margin:0;">Fill out the short form and I'll message you back within a day.</p>
      </div>
      ${cleanImage(images.ctaMain, 'left:42px;top:568px;width:996px;height:1310px;', 'cover', 42, 'top 15%')}
    </div>
  `
}
