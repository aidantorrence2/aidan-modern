...
const OUT_DIR = path.join(__dirname, 'output-v51b')
...
function buildHTML(imageDataMap) {
  ...
  const T = {
    // Notifications arrive (more spaced, more dramatic)
    n1: 1.2,    // "madebyaidan sent you a message" — "hey, I'm a photographer in Manila"
    n2: 3.5,    // "madebyaidan sent a photo" — with photo thumbnail
    n3: 5.5,    // "madebyaidan" — "I'm doing free photo shoots this week"
    n4: 7.5,    // "madebyaidan sent 3 photos" — multiple photo thumbnails
    n5: 10.0,   // "madebyaidan" — "no experience needed, I direct everything"
    n6: 12.5,   // "madebyaidan" — "interested?"
    // Wallpaper shifts to reveal photo beneath
    wallpaperShift: 14.5,
    // Final CTA notification — big, highlighted
    ctaNotif: 16.0,
    // Hold
  }
  ...
  function notif(id, title, body, time, delay, options = {}) {
    ...
    let thumbHTML = ''
    if (thumbnail) {
      thumbHTML = `<div style="width:64px;height:64px;border-radius:10px;overflow:hidden;flex-shrink:0;">
    ...
    if (isPhotos && photoCount) {
      thumbHTML = `<div style="display:flex;gap:4px;flex-shrink:0;">
    ...
    ">
      <!-- IG icon -->
      <div style="width:52px;height:52px;border-radius:14px;overflow:hidden;flex-shrink:0;background:linear-gradient(135deg, #833AB4, #C13584, #E1306C, #F77737, #FCAF45);display:flex;align-items:center;justify-content:center;">
      ...
      <div style="flex:1;min-width:0;">
        <div style="display:flex;align-items:center;justify-content:space-between;margin-bottom:5px;">
          <span style="font-family:${SF};font-size:22px;font-weight:700;color:rgba(255,255,255,0.95);">Instagram</span>
          <span style="font-family:${SF};font-size:18px;color:rgba(255,255,255,0.45);">${time}</span>
        </div>
        <p style="font-family:${SF};font-size:24px;font-weight:600;color:rgba(255,255,255,0.9);margin:0 0 4px;line-height:1.3;">${title}</p>
        ${body ? `<p style="font-family:${SF};font-size:22px;color:${isCTA ? 'rgba(255,255,255,0.85)' : 'rgba(255,255,255,0.6)'};margin:0;line-height:1.35;${isCTA ? 'font-weight:600;' : ''}">${body}</p>` : ''}
      </div>
      ${thumbHTML}
    </div>`
  }
  ...
  const scrollKf = `
    ...
    ${p(T.n4)}% { transform: translateY(-100px); }
    ${p(T.n5)}% { transform: translateY(-250px); }
    ${p(T.n6)}% { transform: translateY(-400px); }
    ${p(T.ctaNotif)}% { transform: translateY(-520px); }
    ${p(T.ctaNotif + 1)}% { transform: translateY(-550px); }
    100% { transform: translateY(-550px); }
  `

  // Wallpaper crossfade — starts with blurred garden, shifts to a clearer photo
  const wallpaperKf = `
    0% { filter: blur(30px) brightness(0.35) saturate(1.2); }
    ${p(T.wallpaperShift)}% { filter: blur(30px) brightness(0.35) saturate(1.2); }
    ${p(T.wallpaperShift + 1.5)}% { filter: blur(15px) brightness(0.25) saturate(1.4); }
...
<body>
  <div class="page">
    ...
    <!-- Status bar -->
    <div style="position:absolute;left:0;right:0;top:0;height:60px;padding:16px 36px 0;display:flex;align-items:center;justify-content:space-between;z-index:20;">
    ...
    <!-- Lock Screen Clock -->
    <div style="position:absolute;left:0;right:0;top:120px;text-align:center;z-index:10;">
      <p style="font-family:${SF};font-size:24px;font-weight:400;color:rgba(255,255,255,0.65);margin:0 0 2px;letter-spacing:0.04em;">Wednesday, March 12</p>
      <p style="font-family:${SF};font-size:160px;font-weight:200;color:#fff;margin:0;line-height:1;letter-spacing:-0.04em;">
    ...
    <div style="position:absolute;top:4px;left:50%;transform:translateX(60px);z-index:25;">
      <div id="badge" style="width:28px;height:28px;border-radius:50%;background:${MANILA_COLOR};display:flex;align-items:center;justify-content:center;opacity:0;transform:scale(0);">
        <span id="badgeNum" style="font-family:${SF};font-size:16px;font-weight:700;color:#fff;">1</span>
      </div>
    </div>
