...
function buildNowPlayingAnimation(images) {
  ...
  // 'sign up below' (index 6) and the last two lyrics get MANILA_COLOR highlight
  const highlightIndices = new Set([6, 7, 8])
  const lastLyricIdx = LYRICS.length - 1
  ...
  LYRICS.forEach((_, idx) => {
    const fadeInAt = lyricStartTime + idx * lyricDuration
    const isHighlight = highlightIndices.has(idx)
    // Last lyric holds bright for 3 extra seconds
    const glowDur = idx === lastLyricIdx ? lyricDuration + 3 : lyricDuration
    const brightColor = isHighlight ? MANILA_COLOR : '#fff'
    const dimColor = isHighlight ? 'rgba(232,68,58,0.5)' : 'rgba(255,255,255,0.35)'
    const glowShadow = isHighlight
      ? '0 0 30px rgba(232,68,58,0.5)'
      : '0 0 20px rgba(255,255,255,0.3)'
    lyricAnimationCSS += `
      .lyric-${idx} {
        animation:
          lyricAppear 0.5s ease-out ${fadeInAt}s forwards,
          lyricGlow${idx} ${glowDur}s ease-in-out ${fadeInAt}s forwards;
      }
      @keyframes lyricGlow${idx} {
        0% { color: ${brightColor}; text-shadow: ${glowShadow}; }
        80% { color: ${brightColor}; text-shadow: ${glowShadow}; }
        100% { color: ${idx === lastLyricIdx ? brightColor : dimColor}; text-shadow: ${idx === lastLyricIdx ? glowShadow : 'none'}; }
      }
    `
  })

  // Progress bar total time matches animation duration
  const totalPlaybackSec = 23
...
async function render() {
  ...
  writeSources({
    ...
    strategy: 'v58 — Apple Music Now Playing screen with cycling album art, lyric reveal, and natural CTA ending',
  ...
  // --- Record the Now Playing animation video ---
  ...
  try {
    ...
    console.log('Rendered 01_apple_music_now_playing.mp4')
  } catch (err) {
