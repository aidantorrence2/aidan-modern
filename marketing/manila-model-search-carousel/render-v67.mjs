...
// Timeline:
// 0-1s: Lock screen appears (time, date)
// 1-2s: Incoming call slides up (ring animation)
// 2-4s: Call ringing (pulse on accept/decline)
// 4s: Accept call (green button press)
// 4.5-5.5s: Transition to "connected" call screen
// 5.5-7s: Call connected, timer ticking, "hey, I saw your work..."
// 7-9s: First photo slides in from side
// 9-11s: Second photo
// 11-13s: Third photo
// 13-15s: Fourth photo
// 15-17s: "can I sign up?"
// 17-20s: CTA text fades in naturally — "sign up below", form info, urgency
// 20-24s: Hold final state
const TOTAL_DURATION = 24
const TOTAL_DURATION_MS = 26000

const p = (t) => ((t / TOTAL_DURATION) * 100).toFixed(2)

// Photos that appear during the "call"
const PHOTOS = [
...
// Chat messages that appear during call
const MESSAGES = [
  ...
  { time: 12.5, text: 'none. here are some photos from recent shoots', sender: 'me', duration: 2.5 },
...
function isPurple(name) { return name.includes('purple') }

function buildCallAnimation(imageDataMap) {
  // Photo gallery keyframes — photos slide in from right during call
  ...
  PHOTOS.forEach((photo, i) => {
    ...
    photoHTML += `
      <div style="position:absolute;left:140px;right:140px;top:${yOffset}px;height:480px;
        border-radius:20px;overflow:hidden;box-shadow:0 12px 40px rgba(0,0,0,0.5);
        opacity:0;z-index:${15 + i};
        animation:photoSlide${i} ${TOTAL_DURATION}s linear forwards;">
        <img src="${imageDataMap[photo.file]}" style="${imgStyle}"/>
      </div>
    `
  })

  // Message bubble keyframes
  let messageKeyframes = ''
  let messageHTML = ''

  MESSAGES.forEach((msg, i) => {
    const isMe = msg.sender === 'me'
    const bubbleBg = isMe ? '#34C759' : 'rgba(255,255,255,0.15)'
    const textColor = isMe ? '#fff' : '#fff'
    const align = isMe ? 'right:60px' : 'left:60px'
    const maxW = 680
    const yPos = 340 + (i * 95)
    ...
    messageHTML += `
      <div style="position:absolute;${align};top:${yPos}px;max-width:${maxW}px;z-index:12;
        ...
        <div style="background:${bubbleBg};border-radius:22px;padding:18px 26px;
          ${isMe ? 'border-bottom-right-radius:6px;' : 'border-bottom-left-radius:6px;'}">
          <p style="font-family:${SF};font-size:36px;font-weight:500;color:${textColor};margin:0;line-height:1.35;">${msg.text}</p>
        </div>
      </div>
    `
  })

  // CTA content — appears naturally after "how do I sign up?"
  const ctaStart = 18.5
...
<style>
  * { box-sizing:border-box;margin:0;padding:0; }
  ...
  /* Ring pulse on caller photo */
  @keyframes ringPulse {
    0%, 50% { box-shadow:0 0 0 0 rgba(52,199,89,0.4); }
    25% { box-shadow:0 0 0 25px rgba(52,199,89,0); }
    75% { box-shadow:0 0 0 25px rgba(52,199,89,0); }
  ...
  @keyframes acceptPress {
    ...
    ${p(4.0)}% { transform:scale(0.85); background:#34C759; }
    ${p(4.3)}% { transform:scale(1.1); background:#34C759; }
  ...
  /* Call header transition from "incoming" to "connected" */
  ...
  /* Caller photo stays, shrinks to top bar after connect */
  @keyframes callerPhotoShrink {
    0%, ${p(4.5)}% {
      width:200px;height:200px;border-radius:100px;
      top:280px;left:${(WIDTH - 200) / 2}px;
    }
    ${p(5.5)}% {
  ...
  /* Name moves to top after connect */
  @keyframes nameMove {
    0%, ${p(4.5)}% {
      top:510px;left:0;right:0;text-align:center;font-size:52px;
    }
    ${p(5.5)}% {
      top:108px;left:150px;right:auto;text-align:left;font-size:36px;
    }
    100% {
      top:108px;left:150px;right:auto;text-align:left;font-size:36px;
    }
  ...
  /* Subtle green glow at top during ring */
  @keyframes greenGlow {
    0%, ${p(1.0)}% { opacity:0; }
    ${p(1.5)}% { opacity:1; }
    ${p(2.0)}% { opacity:0.5; }
...
<div id="root" style="width:${WIDTH}px;height:${HEIGHT}px;position:relative;overflow:hidden;background:#000;">
  ...
  <div style="position:absolute;inset:-40px;z-index:1;
    ...
    <img src="${imageDataMap[PHOTOS[0].file]}" style="width:130%;height:130%;object-fit:cover;object-position:center center;display:block;margin:-15% 0 0 -15%;"/>
  </div>

  <!-- ===== GREEN GLOW AT TOP (during ring) ===== -->
  ...
  <div style="position:absolute;inset:0;z-index:50;
    ...
    <div style="position:absolute;inset:0;background:linear-gradient(180deg, #1c1c1e 0%, #000 100%);">
      <!-- Status bar -->
      <div style="padding:60px 36px 0;display:flex;justify-content:space-between;align-items:center;">
      ...
      <div style="text-align:center;margin-top:120px;">
        <svg width="28" height="34" viewBox="0 0 28 34" fill="none" style="margin:0 auto;">
      ...
      <!-- Time -->
      <p style="font-family:${SF};font-size:120px;font-weight:200;color:#fff;text-align:center;margin:40px 0 0;letter-spacing:-2px;">9:41</p>
      <!-- Date -->
      <p style="font-family:${SF};font-size:28px;font-weight:400;color:rgba(255,255,255,0.6);text-align:center;margin:8px 0 0;">Wednesday, March 12</p>
    </div>
  </div>
  ...
  <div style="position:absolute;inset:0;z-index:10;
    ...
    <!-- Status bar -->
    <div style="padding:60px 36px 0;display:flex;justify-content:space-between;align-items:center;position:relative;z-index:20;">
      <span style="font-family:${SF};font-size:20px;font-weight:600;color:#fff;">9:41</span>
      <div style="display:flex;align-items:center;gap:6px;">
        <svg width="20" height="14" viewBox="0 0 18 12"><rect x="0" y="3" width="3" height="9" rx="1" fill="#fff"/><rect x="5" y="2" width="3" height="10" rx="1" fill="#fff"/><rect x="10" y="0" width="3" height="12" rx="1" fill="#fff"/><rect x="15" y="0" width="3" height="12" rx="1" fill="#fff" opacity="0.3"/></svg>
        <svg width="30" height="15" viewBox="0 0 27 13"><rect x="0" y="0" width="23" height="13" rx="3.5" stroke="#fff" stroke-width="1" fill="none"/><rect x="2" y="2" width="18" height="9" rx="2" fill="#fff"/></svg>
    ...
    <!-- Caller photo (animated position) -->
    ...
    <!-- Caller name (animated position) -->
    <div style="position:absolute;z-index:15;
      top:510px;left:0;right:0;text-align:center;
    ...
    <!-- "Incoming call" / "Connected" text -->
    <p style="position:absolute;top:570px;left:0;right:0;text-align:center;z-index:15;
      font-family:${SF};font-size:24px;font-weight:400;color:rgba(255,255,255,0.6);
      animation:incomingText ${TOTAL_DURATION}s linear forwards;">incoming call...</p>

    <!-- Connected status + timer -->
    <div style="position:absolute;top:148px;left:150px;z-index:15;
      opacity:0;animation:connectedText ${TOTAL_DURATION}s linear forwards;">
      <p id="callTimer" style="font-family:${SF};font-size:22px;font-weight:400;color:rgba(52,199,89,0.9);margin:0;">0:00</p>
    </div>

    <!-- Accept / Decline buttons (during ring) -->
    <div style="position:absolute;bottom:${SAFE_BOTTOM + 160}px;left:0;right:0;display:flex;justify-content:center;gap:140px;z-index:20;">
      <!-- Decline -->
      <div style="text-align:center;animation:declineBtn ${TOTAL_DURATION}s linear forwards;">
        <div style="width:80px;height:80px;border-radius:40px;background:rgba(255,59,48,0.25);
        ...
        <p style="font-family:${SF};font-size:18px;color:rgba(255,255,255,0.6);margin:12px 0 0;">Decline</p>
      </div>
      <!-- Accept -->
      <div style="text-align:center;">
        <div style="width:80px;height:80px;border-radius:40px;
          display:flex;align-items:center;justify-content:center;
          animation:acceptPress ${TOTAL_DURATION}s linear forwards;background:rgba(52,199,89,0.25);">
          <svg width="36" height="36" viewBox="0 0 24 24" fill="none">
            <path d="M20.01 15.38c-1.23 0-2.42-.2-3.53-.56-.35-.12-.74-.03-1.01.24l-1.57 1.97c-2.83-1.35-5.48-3.9-6.89-6.83l1.95-1.66c.27-.28.35-.67.24-1.02-.37-1.11-.56-2.3-.56-3.53 0-.54-.45-.99-.99-.99H4.19C3.65 3 3 3.24 3 3.99 3 13.28 10.73 21 20.01 21c.71 0 .99-.63.99-1.18v-3.45c0-.54-.45-.99-.99-.99z" fill="#34C759"/>
          </svg>
        </div>
        <p style="font-family:${SF};font-size:18px;color:rgba(255,255,255,0.6);margin:12px 0 0;
          animation:declineBtn ${TOTAL_DURATION}s linear forwards;">Accept</p>
      </div>
    </div>

    <!-- Call action buttons (appear after connect) -->
    <div style="position:absolute;bottom:${SAFE_BOTTOM + 60}px;left:0;right:0;z-index:20;
      ...
      <div style="display:flex;justify-content:center;gap:50px;">
        ${['mute', 'keypad', 'speaker', 'end'].map(action => {
          ...
          const size = action === 'end' ? 76 : 60
          const iconColor = action === 'end' ? '#fff' : 'rgba(255,255,255,0.7)'
          const icons = {
          ...
          return `<div style="text-align:center;">
            <div style="width:${size}px;height:${size}px;border-radius:${size / 2}px;background:${bg};
              display:flex;align-items:center;justify-content:center;">
              <svg width="28" height="28" viewBox="0 0 24 24" fill="none">${icons[action]}</svg>
            </div>
            <p style="font-family:${SF};font-size:14px;color:rgba(255,255,255,0.5);margin:8px 0 0;">${action}</p>
          </div>`
        }).join('')}
      </div>
    </div>

    <!-- ===== MESSAGE BUBBLES (appear during call) ===== -->
    ${messageHTML}

    <!-- ===== PHOTO GALLERY (slides through during call) ===== -->
    ${photoHTML}

    <!-- ===== CTA — natural ending ===== -->
    <div style="position:absolute;left:0;right:0;top:380px;text-align:center;z-index:25;
      opacity:0;animation:ctaIn ${TOTAL_DURATION}s linear forwards;">

      <!-- "MANILA" huge -->
      <p style="font-family:${SF};font-size:140px;font-weight:900;letter-spacing:0.14em;color:#fff;margin:0;
        text-shadow:0 4px 60px rgba(232,68,58,0.3), 0 2px 20px rgba(0,0,0,0.8);">MANILA</p>

      <!-- PHOTO SHOOT -->
      <p style="font-family:${SF};font-size:32px;font-weight:300;color:rgba(255,255,255,0.85);margin:4px 0 0;letter-spacing:0.3em;">PHOTO SHOOT</p>

      <!-- Sign up below -->
      <p style="font-family:${SF};font-size:64px;font-weight:800;color:#fff;margin:50px 0 0;">Sign up below.</p>

      <!-- 60-second form -->
      <p style="font-family:${SF};font-size:30px;font-weight:400;color:rgba(255,255,255,0.7);margin:16px 0 0;">60-second form. I'll message<br/>you back within a day.</p>

      <!-- Urgency badge -->
      <div style="display:inline-flex;align-items:center;gap:10px;padding:14px 28px;border-radius:14px;
        background:rgba(232,68,58,0.15);border:1px solid rgba(232,68,58,0.4);margin:30px 0 0;">
        <div style="width:10px;height:10px;border-radius:50%;background:${MANILA_COLOR};"></div>
        <span style="font-family:${SF};font-size:24px;font-weight:600;color:rgba(255,255,255,0.85);">Limited spots this month</span>
      </div>

      <!-- Handle -->
      <p style="font-family:${SF};font-size:22px;font-weight:500;color:rgba(255,255,255,0.35);margin:24px 0 0;">@${HANDLE}</p>
    </div>
...
<script>
  // Call timer — starts at ~5s into the animation
...
async function render() {
  ...
  writeSources({
    ...
    strategy: 'v67 — iPhone incoming call animation with photo gallery and natural CTA',
  ...
  // Convert webm → mp4 (single file, no concat)
