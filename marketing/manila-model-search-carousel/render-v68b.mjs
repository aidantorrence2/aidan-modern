...
const OUT_DIR = path.join(__dirname, 'output-v68b')
...
async function render() {
  ...
  writeSources({
    ...
    strategy: 'v68b — bigger text variant — IG DMs broken animation, messages fail then photographer thread appears',
  ...
  const finalMp4 = path.join(OUT_DIR, '01_ig_dms_broken_bigger.mp4')

  try {
    execSync(`ffmpeg -y -i "${srcVideo}" -c:v libx264 -pix_fmt yuv420p -r 30 -an "${finalMp4}"`, { stdio: 'pipe' })
    fs.unlinkSync(srcVideo)
    console.log('Rendered 01_ig_dms_broken_bigger.mp4')
  } catch (err) {
