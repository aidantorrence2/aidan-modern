...
const OUT_DIR = path.join(__dirname, 'output-v68c')
...
async function render() {
  ...
  writeSources({
    ...
    strategy: 'v68c — IG DMs broken, different copy angle',
  ...
  console.log('Recording IG DMs broken animation (v68c)...')
  ...
  const finalMp4 = path.join(OUT_DIR, '01_ig_dms_broken_alt.mp4')

  try {
    execSync(`ffmpeg -y -i "${srcVideo}" -c:v libx264 -pix_fmt yuv420p -r 30 -an "${finalMp4}"`, { stdio: 'pipe' })
    fs.unlinkSync(srcVideo)
    console.log('Rendered 01_ig_dms_broken_alt.mp4')
  } catch (err) {
