...
const OUT_DIR = path.join(__dirname, 'output-v71')
...
function buildHTML(imageDataMap) {
  ...
  const T = {
    ...
    // Phase 3: DM replies flooding in (4.5-18s)
    firstReply: 4.5,
    replyInterval: 1.2,  // seconds between each reply

    // Phase 4: Open one reply → photographer response + CTA (18-26s)
    openThread: 18.5,
    photographerReply1: 19.5,
    photographerReply2: 20.8,
    ctaMsg: 22.0,
    ctaHandle: 23.2,
    // Hold until 26s
  }
...
<body>
    ...
    <div class="thread-view" id="threadView">
      ...
      <div class="thread-chat">
        <div class="thread-scroll" id="threadScroll">
          ...
          <!-- The story reply reference -->
          <div class="msg-row recv" id="threadReplyRef" style="opacity:0;">
          ...
          <div class="msg-row recv" id="threadPhotogReply1" style="opacity:0;">
            <div class="bubble recv">
              <p>hey thanks!! I shot those myself actually 📸</p>
            </div>
          </div>

          <div class="msg-row recv" id="threadPhotogReply2" style="opacity:0;">
            <div class="bubble recv">
              <p>I'm doing free shoots in Manila right now — no catch, just building my portfolio</p>
            </div>
          </div>
  ...
  <script>
    ...
    const threadMsgs = [
      { id: 'threadReplyRef',      t: ${T.openThread * 1000 + 400} },
      { id: 'threadPhotogReply1',  t: ${T.photographerReply1 * 1000} },
      { id: 'threadPhotogReply2',  t: ${T.photographerReply2 * 1000} },
      { id: 'threadCta',           t: ${T.ctaMsg * 1000} },
      { id: 'threadHandle',        t: ${T.ctaHandle * 1000} },
    ]

    threadMsgs.forEach(({ id, t }) => {
      setTimeout(() => {
        const el = document.getElementById(id)
        if (el) {
...
async function render() {
  ...
  writeSources({
    ...
    strategy: 'v71 — story reply flood animation',
  ...
  const finalMp4 = path.join(OUT_DIR, '01_story_replies.mp4')

  try {
    execSync(`ffmpeg -y -i "${srcVideo}" -c:v libx264 -pix_fmt yuv420p -r 30 -an "${finalMp4}"`, { stdio: 'pipe' })
    fs.unlinkSync(srcVideo)
    console.log('Rendered 01_story_replies.mp4')
  } catch (err) {
