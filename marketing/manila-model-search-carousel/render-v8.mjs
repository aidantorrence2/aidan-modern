...
const OUT_DIR = path.join(__dirname, 'output-v8')
...
function slideOne(images) {
  return `
    <div style="width:${WIDTH}px;height:${HEIGHT}px;position:relative;overflow:hidden;background:#efe6dc;">
      ...
      ${framed(images.hero, 'left:58px;top:712px;width:964px;height:876px;', 'cover')}
    </div>
  `
}
...
function slideFive(images) {
  return `
    <div style="width:${WIDTH}px;height:${HEIGHT}px;position:relative;overflow:hidden;background:#0f1721;">
      ...
      ${safeShade(true)}
      ${topLabel(true)}
      <div style="position:absolute;left:58px;top:132px;width:964px;padding:34px 34px 32px;border-radius:34px;background:rgba(6,10,15,0.9);border:1px solid rgba(255,255,255,0.12);box-shadow:0 30px 62px rgba(0,0,0,0.24);">
        ...
        <h2 style="font-family:${DISPLAY};font-size:80px;font-weight:700;line-height:0.9;color:#fff;margin:0 0 18px;">Tap sign up to apply.</h2>
      ...
      ${framed(images.ctaMain, 'left:58px;top:688px;width:964px;height:900px;', 'cover')}
    </div>
  `
}
