...
const OUT_DIR = path.join(__dirname, "output-31f-subic")
...
function buildHTML(imageDataMap) {
  ...
  var titleWords = [
    { word: 'SUBIC', color: TC.I, delay: 0.3 },
...
'        <div style="font-size:18px;font-weight:900;color:#00f0f0;letter-spacing:3px;text-shadow:0 0 12px rgba(0,240,240,0.5);">SUBIC FREE PHOTO SHOOT</div>\n' +
'      </div>\n' +
...
async function render() {
  ...
  writeSources({
    ...
    strategy: 'v31f-subic — Word-block title with separator lines between words',
    safeBottomPixels: SAFE_BOTTOM,
    photos: PROOF_PHOTOS,
  })

  var browser = await chromium.launch()
  console.log('Recording Tetris-themed reel (v31f-subic — word blocks with separator lines)...')
  ...
  var dstVideo = path.join(OUT_DIR, 'subic-tetris-v31f.mp4')
  ...
  try {
    ...
    console.log('Rendered subic-tetris-v31f.mp4')
  } catch (err) {
  ...
  child_process.execSync('cp "' + dstVideo + '" "' + path.join(reelsDir, 'subic-tetris-v31f.mp4') + '"')
  console.log('Copied to reels/subic-tetris-v31f.mp4')
