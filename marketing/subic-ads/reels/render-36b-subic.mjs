...
var OUT_DIR = path.join(__dirname, 'output-36b-subic');
...
async function main() {
  console.log('=== Subic BTS Lightbox Reel v36b-subic ===');
  resetOutputDir();
  ...
  var finalMp4 = path.join(OUT_DIR, 'subic-bts-lightbox-v36b.mp4');
  execSync(
  ...
  execSync('cp "' + finalMp4 + '" "' + path.join(reelsDir, 'subic-bts-lightbox-v36b.mp4') + '"');

  var sz = statSync(finalMp4);
  console.log('Final: ' + (sz.size / (1024 * 1024)).toFixed(1) + ' MB');
  console.log('Copied to reels/subic-bts-lightbox-v36b.mp4');
  console.log('=== Done ===');
}
